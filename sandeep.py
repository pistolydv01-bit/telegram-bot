...
def _perform_action(driver, elem, action_type, text_value):
    ...
    if action_type == "click":    
        ...
        # Keys.TAB हटा दिया गया है ताकि OTP popup ट्रिगर हो सके
    elif action_type == "file":    
        elem.send_keys(os.path.abspath(text_value))    
    return elem

# --- NEW: Mobile OTP Popup Detector ---
def detect_mobile_otp_popup(driver, timeout=5):
    ...
    otp_input_xpaths = [
        ...
        "//div[contains(@class,'modal') or @role='dialog']//input"
    ]
    while time.time() - start_time < timeout:
        try:
            driver.switch_to.default_content()
...
def fill_initial_rtps_form(driver, user_data, service_type, photo_path, chat_id=None, user_id=None):    
    ...
    # --- UPDATED EMAIL & MOBILE OTP TRIGGER FLOW ---
    if user_data.get("email"):    
        try:    
            ...
            # केवल click करें (type नहीं) ताकि OTP popup ट्रिगर हो सके
...
def continue_rtps_form_after_mobile_otp(driver, user_data, service_type, photo_path, chat_id=None, user_id=None):
    """मोबाइल OTP सत्यापित होने के बाद फॉर्म के बचे हुए हिस्से को भरने के लिए"""
    try:
        if user_data.get("email"):
            try:
                find_and_interact(driver, [
