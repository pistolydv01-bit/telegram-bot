...
2. Aadhaar number बिल्कुल extract/store मत करो; Aadhaar बाद में Annexure page पर upload होगा।
3. कोई value अनुमान से मत बनाओ।
4. जानकारी उपलब्ध न हो तो empty string "" रखो।
5. नीचे दिए JSON के keys बिल्कुल न बदलो।
6. कोई extra key मत जोड़ो।
7. केवल valid JSON return करो।
8. JSON के बाहर कोई explanation मत दो।
...
def fill_initial_rtps_form(driver, user_data, service_type, photo_path, chat_id=None, user_id=None):
    ...
    # Aadhaar number is intentionally NOT filled here.
    # It will be uploaded later on the Annexure page.
...
def route_to_next_step(bot, chat_id, user_id, driver, user_dir):
    ...
    # No Aadhaar OTP/final-submit fallback here. The expected next stage is Annexure.
    try:
        if open_annexure_page_after_proceed(driver, chat_id, timeout=8):
            if wait_for_annexure_form(driver, timeout=10):
                session = active_user_sessions.get(chat_id)
                if session:
                    session["waiting_for"] = "ANNEXURE_UPLOAD"
                    session["awaiting_annexure"] = True
                    session["is_processing"] = False
                bot.send_message(chat_id, "📎 Attach Annexure page खुल गया है। Aadhaar PDF/JPG/PNG भेजें।")
                bot.set_state(user_id, RTPSState.annexure_upload, chat_id)
                return
    except Exception:
        pass
    bot.send_message(chat_id, "⚠️ वर्तमान चरण की स्पष्ट पहचान नहीं हो सकी। कृपया screenshot देखें।")
...
def process_service_type(message):
    ...
    bot.send_message(message.chat.id, f"सर्विस '{service}' चुनी गई। विवरण नीचे दिए गए JSON प्रारूप में भरें (Aadhaar number के बिना):\n\n`{templates[service]}`", parse_mode="Markdown")      
...
def open_annexure_page_after_proceed(driver, chat_id, timeout=20):
    """After the initial CAPTCHA Proceed + RTPS alert, open Attach Annexure."""
    attach_xpaths = [
        "//input[@value='Attach Annexure']",
        "//button[contains(normalize-space(.),'Attach Annexure')]",
        "//a[contains(normalize-space(.),'Attach Annexure')]",
        "//*[self::button or self::input or self::a][contains(translate(normalize-space(.),'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'attach annexure')]"
    ]
    deadline = time.time() + timeout
    last_error = None
            ...
            for xp in attach_xpaths:
                for elem in driver.find_elements(By.XPATH, xp):
                    ...
                    try:
                        ...
                        driver.execute_script("arguments[0].click();", elem)
                    logging.info("✅ Attach Annexure clicked after CAPTCHA/alert")
                    time.sleep(1.5)
                    send_step_screenshot(driver, chat_id, "ATTACH_ANNEXURE_PAGE")
                    return True
        except Exception as e:
            last_error = e
        time.sleep(0.4)

    raise TimeoutException(f"Attach Annexure button नहीं मिला: {last_error!r}")


def wait_for_annexure_form(driver, timeout=20):
    """Wait until the actual ATTACH ENCLOSURE page is visible."""
    markers = [
        "//*[contains(translate(normalize-space(.),'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'attach enclosure')]",
        "//*[contains(normalize-space(.),'Enclosure(s)')]",
...
def _select_annexure_option(select_elem, wanted_terms):
    wanted_terms = [" ".join(str(x).split()).casefold() for x in wanted_terms]
    options = select_elem.find_elements(By.TAG_NAME, "option")
    for opt in options:
        text = " ".join((opt.text or "").split()).strip()
        norm = text.casefold()
        if any(term in norm for term in wanted_terms):
            ...
            return text
    raise NoSuchElementException(f"Annexure option not found: {wanted_terms!r}")


def attach_aadhaar_and_save_annexure(driver, aadhaar_path, chat_id):
    """Upload Aadhaar in the first enclosure row, select 'अन्याय अभिलेख' in the last row, then Save Annexure."""
    driver.switch_to.default_content()

    # First row: Identification document of applicant -> Aadhaar Card.
    first_row_xpaths = [
        "//tr[.//*[contains(normalize-space(.),'Identification document of applicant') or contains(normalize-space(.),'आवेदक का पहचान दस्तावेज')]][1]",
        "//*[contains(normalize-space(.),'Identification document of applicant') or contains(normalize-space(.),'आवेदक का पहचान दस्तावेज')]/ancestor::tr[1]"
    ]
    first_row = None
    ...
    if first_row is None:
        raise NoSuchElementException("Aadhaar identification enclosure row नहीं मिली")

    selects = first_row.find_elements(By.XPATH, ".//select")
    if not selects:
        raise NoSuchElementException("Aadhaar enclosure dropdown नहीं मिला")
    _select_annexure_option(selects[0], ["आधार कार्ड", "aadhaar card", "aadhar card"])

    file_inputs = first_row.find_elements(By.XPATH, ".//input[@type='file']")
    if not file_inputs:
        raise NoSuchElementException("Aadhaar enclosure file input नहीं मिला")
    file_input = next((e for e in file_inputs if e.is_displayed() and e.is_enabled()), file_inputs[0])
    try:
        file_input.send_keys(os.path.abspath(aadhaar_path))
    except Exception:
        # RTPS may keep the file input visually hidden; temporarily make it interactable.
        driver.execute_script(
            "arguments[0].style.display='block'; arguments[0].style.visibility='visible';",
            file_input
        )
        file_input.send_keys(os.path.abspath(aadhaar_path))
    logging.info("✅ Aadhaar file attached in Identification document row")

    # Last/Other record row: select 'अन्याय अभिलेख'. No file is attached there.
    other_row_xpaths = [
        "//tr[.//*[contains(normalize-space(.),'अन्याय अभिलेख') or contains(normalize-space(.),'अन्य अभिलेख') or contains(normalize-space(.),'Other Record')]][1]",
        "//*[contains(normalize-space(.),'अन्याय अभिलेख') or contains(normalize-space(.),'अन्य अभिलेख') or contains(normalize-space(.),'Other Record')]/ancestor::tr[1]"
    ]
    other_row = None
    for xp in other_row_xpaths:
        try:
            rows = driver.find_elements(By.XPATH, xp)
            for row in rows:
                if row.is_displayed():
    ...
    if other_row is not None:
        other_selects = other_row.find_elements(By.XPATH, ".//select")
        if other_selects:
            _select_annexure_option(other_selects[0], ["अन्याय अभिलेख", "अन्य अभिलेख", "other record", "other"])
            logging.info("✅ 'अन्याय अभिलेख' selected")
    else:
        # Screenshot-based fallback: the fourth visible enclosure dropdown is Other Record.
        visible_selects = [
            e for e in driver.find_elements(By.XPATH, "//select")
            if e.is_displayed() and e.is_enabled()
        ]
        if len(visible_selects) >= 4:
            _select_annexure_option(visible_selects[3], ["अन्याय अभिलेख", "अन्य अभिलेख", "other record", "other"])
            logging.info("✅ 'अन्याय अभिलेख' selected via fourth visible dropdown")
        else:
            raise NoSuchElementException("'अन्याय अभिलेख' dropdown नहीं मिला")

    send_step_screenshot(driver, chat_id, "ANNEXURE_AADHAAR_FILLED")

    save_xpaths = [
        "//input[@value='Save Annexure']",
        "//button[contains(normalize-space(.),'Save Annexure')]",
        "//*[self::button or self::input][contains(translate(normalize-space(.),'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'save annexure')]"
    ]
        ...
        for elem in driver.find_elements(By.XPATH, xp):
            if elem.is_displayed() and elem.is_enabled():
                ...
                try:
                    ...
                    driver.execute_script("arguments[0].click();", elem)
                time.sleep(2)
                logging.info("✅ Save Annexure clicked")
                return True

    raise NoSuchElementException("Save Annexure button नहीं मिला")


@bot.message_handler(state=RTPSState.captcha_input)
def process_captcha_input(message):
    """Handle the single CAPTCHA, then OK the RTPS alert and open Annexure."""
    chat_id = message.chat.id
    user_id = message.from_user.id
    if not user_is_allowed(user_id):
        return
    ...
    try:
        # 1) Fill the current visible CAPTCHA.
        fill_visible_captcha_input(driver, captcha_text, timeout=20)
        send_step_screenshot(driver, chat_id, "CAPTCHA_CODE_FILLED_FULL_SCREEN")
        ...
        find_and_interact(
            ...
            [
                ...
                "//input[@value='Process']",
                "//input[contains(@id,'btnProcess')]"
            ],
            action_type="click",
            timeout=20,
            chat_id=chat_id
        )
        logging.info("✅ CAPTCHA Proceed clicked")
        send_step_screenshot(driver, chat_id, "AFTER_CAPTCHA_PROCEED")
        time.sleep(1)

        # 3) The real RTPS browser alert says to verify name/mobile/email.
        #    Accept it; do NOT open Aadhaar OTP.
        alert_handled, alert_text = accept_rtps_verification_alert(driver, timeout=12)
        if alert_handled:
            logging.info("✅ RTPS verification alert accepted: %s", mask_sensitive_data(alert_text))
        else:
            logging.info("ℹ️ No browser verification alert detected after Proceed")

        click_post_proceed_ok_if_present(driver, chat_id=chat_id, timeout=4)
        time.sleep(1.5)

        # 4) Wait for the summary/review page and click Attach Annexure.
        if not open_annexure_page_after_proceed(driver, chat_id, timeout=25):
            raise TimeoutException("Attach Annexure page open नहीं हुआ")

        if not wait_for_annexure_form(driver, timeout=25):
            raise TimeoutException("ATTACH ENCLOSURE page नहीं खुला")
        ...
        send_step_screenshot(driver, chat_id, "ANNEXURE_PAGE_READY")
        bot.send_message(
            chat_id,
            "✅ CAPTCHA verify हुआ और RTPS alert पर OK कर दिया गया।\n\n"
            "📎 अब Attach Annexure page खुल गया है।\n"
            "🪪 अपना Aadhaar Card PDF/JPG/PNG भेजें।\n"
            "➡️ Bot उसे 'Identification document of applicant' में Aadhaar Card के रूप में लगाएगा,\n"
            "और 'अन्याय अभिलेख' select करके Save Annexure करेगा।"
        )
        bot.set_state(user_id, RTPSState.annexure_upload, chat_id)
        ...
        session["is_processing"] = False
        session["last_activity"] = time.time()
        logging.exception("Captcha -> Annexure processing error")
        bot.send_message(chat_id, f"❌ CAPTCHA/Annexure प्रक्रिया में त्रुटि:\n{e}")
        send_error_screenshot(chat_id, driver, "❌ CAPTCHA/Annexure Error", e)


@bot.message_handler(content_types=['photo', 'document'], state=RTPSState.annexure_upload)
def process_annexure_upload(message):
    """Receive Aadhaar file only at the Annexure stage and save the enclosure."""
    chat_id = message.chat.id
    user_id = message.from_user.id
    if not user_is_allowed(user_id):
        return
    ...
    if not session:
        bot.delete_state(chat_id=chat_id, user_id=user_id)
        bot.send_message(chat_id, "⚠️ सेशन समाप्त हो गया है। /start से पुनः शुरू करें।")
        return

    if not user_locks[chat_id].acquire(blocking=False):
        bot.send_message(chat_id, "⏳ Annexure upload की पिछली प्रक्रिया अभी चल रही है।")
        return
    ...
    try:
        ...
        if message.document:
            ...
            bot.send_message(chat_id, "❌ केवल PDF/JPG/JPEG/PNG भेजें।")
            return

        if file_info.file_size and file_info.file_size > MAX_FILE_SIZE_MB * 1024 * 1024:
            bot.send_message(chat_id, "❌ Aadhaar file का आकार बहुत बड़ा है।")
            return

        _, ext = os.path.splitext(file_info.file_path)
        ext = ext.lower()
        if ext not in ALLOWED_DOC_EXTS:
            ext = ".jpg" if message.photo else ""
        if ext not in ALLOWED_DOC_EXTS:
            bot.send_message(chat_id, "❌ Aadhaar केवल PDF/JPG/JPEG/PNG में भेजें।")
            return

        aadhaar_path = os.path.join(user_dir, f"aadhaar_annexure{ext}")
        downloaded = bot.download_file(file_info.file_path)
        ...
        if not valid:
            bot.send_message(chat_id, f"❌ Aadhaar file त्रुटि: {err_msg}")
            return

        session["is_processing"] = True
        session["last_activity"] = time.time()
        bot.send_message(chat_id, "⏳ Aadhaar को Annexure में लगाया जा रहा है...")

        attach_aadhaar_and_save_annexure(driver, aadhaar_path, chat_id)

        session["waiting_for"] = "AFTER_ANNEXURE_SAVE"
        session["awaiting_annexure"] = False
        session["is_processing"] = False
        session["last_activity"] = time.time()

        bot.send_message(
            chat_id,
            "✅ Aadhaar Card upload हो गया।\n"
            "✅ 'अन्याय अभिलेख' select हो गया।\n"
            "✅ Save Annexure पर click हो गया।\n\n"
            "📸 अगला page का screenshot भेज दिया गया है।"
        )
        send_step_screenshot(driver, chat_id, "AFTER_SAVE_ANNEXURE")

    except Exception as e:
        session["is_processing"] = False
        session["last_activity"] = time.time()
        logging.exception("Annexure upload error")
        bot.send_message(chat_id, f"❌ Aadhaar Annexure upload में समस्या:\n{e}")
