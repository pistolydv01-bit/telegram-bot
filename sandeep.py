...
# --- Updated: Broad Mobile OTP Popup Detector ---
def detect_mobile_otp_popup(driver, timeout=5):
    start_time = time.time()
    otp_input_xpaths = [
        "//input[@id='mobile_otp']",
        ...
        "//div[contains(@class,'modal') or @role='dialog' or contains(@class, 'popup')]//input"
    ]
    while time.time() - start_time < timeout:
        try:
            driver.switch_to.default_content()
    ...
    while time.time() - start_time < timeout:
        try:
            ...
            for xp in mobile_otp_xpaths:
                elems = driver.find_elements(By.XPATH, xp)
                if any(e.is_displayed() for e in elems): return "MOBILE_OTP"
...
def process_mobile_otp_input(message):    
    ...
    try:    
        find_and_interact(driver, [
            ...
            "//input[contains(@name, 'mobileOtp')]",
            "//input[contains(translate(@id,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'mobileotp')]"
        ], "type", otp_code, chat_id=chat_id)    
        
        validate_btn_xpaths = [
            "//button[contains(text(), 'Verify') or contains(text(), 'सत्यापित')]", 
            "//input[@value='Validate']",
            "//button[contains(normalize-space(.), 'Validate')]",
            "//a[contains(normalize-space(.), 'Validate')]"
        ]
        find_and_interact(driver, validate_btn_xpaths, chat_id=chat_id)    
