...
def accept_rtps_verification_alert(driver, timeout=30):
    ...
    while time.time() < deadline:
        try:
            ...
            # CAPTCHA failure can be reported ONLY in the JavaScript alert.
            # The old code classified an unknown alert as OTHER, which caused
            # the bot to wait for a page that would never arrive and then send
            # the unchanged CAPTCHA screenshot as NEXT_PAGE.
            ...
            if any(marker in lower or marker in text for marker in captcha_error_markers):
                try:
                    alert.accept()
                except Exception:
                    pass
                logging.warning(
                    "❌ RTPS CAPTCHA alert मिला: %s",
                    mask_sensitive_data(text)
                )
                return True, text, "CAPTCHA_ERROR"
...
def _probe_captcha_proceed_candidates(driver, captcha_elem=None):
    """Find the real visible Proceed control associated with the CAPTCHA form.

    RTPS reuses id=submit_btn on several pages, so selecting by id alone can
    click the wrong submit control. This helper prefers a Proceed button/input
    whose nearest form also contains the currently visible CAPTCHA input.
    """
    driver.switch_to.default_content()
    xpaths = [
        ...
        "//input[translate(normalize-space(@value),'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz')='proceed']",
                ...
                try:
                    ...
                    info = driver.execute_script("""
                        ...
                        return {
                            ...
                            formMethod: f ? (f.getAttribute('method') || '') : '',
                            sameFormCaptcha: !!(f && c && f.contains(c))
                        };
                    """, elem, captcha_elem)
                    candidates.append((bool(info.get('sameFormCaptcha')), elem, info))
                except Exception:
                    continue
        except Exception:
            continue
    ...
    # Same-form CAPTCHA candidate is authoritative; otherwise use the first
    # visible enabled Proceed control.
    candidates.sort(key=lambda item: 0 if item[0] else 1)
    same_form, elem, info = candidates[0]
    logging.info(
        "🎯 RTPS Proceed probe: sameFormCaptcha=%s tag=%s id=%s text=%r type=%r name=%r onclick=%r formId=%r formAction=%r formMethod=%r",
        same_form, info.get('tag'), info.get('id'), info.get('text'),
        info.get('type'), info.get('name'), info.get('onclick'),
        info.get('formId'), info.get('formAction'), info.get('formMethod')
    )
    return elem, info


def _captcha_page_still_active(driver):
    """Return True only when the visible CAPTCHA input is still on screen."""
    try:
        driver.switch_to.default_content()
        elems = driver.find_elements(By.XPATH, "//input[@id='captchaAnswer' or contains(translate(@id,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'captcha') or contains(translate(@name,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'captcha')]")
        return any(e.is_displayed() for e in elems)
    except Exception:
        return False


def _rtps_state_snapshot(driver):
    """Small diagnostic snapshot; sensitive input values are intentionally omitted."""
    try:
        driver.switch_to.default_content()
        return driver.execute_script("""
            const c = document.querySelector('#captchaAnswer, input[id*='captcha' i], input[name*='captcha' i]');
            const b = [...document.querySelectorAll('button,input[type=submit],input[type=button]')].find(e => {
                const t = (e.innerText || e.value || '').trim().toLowerCase();
                return e.offsetParent !== null && t.includes('proceed');
            });
            const f = b ? b.closest('form') : null;
            return {
                url: location.href,
                readyState: document.readyState,
                title: document.title || '',
                captchaPresent: !!c,
                captchaVisible: !!(c && c.offsetParent !== null),
                proceedPresent: !!b,
                proceedDisabled: !!(b && b.disabled),
                proceedType: b ? (b.getAttribute('type') || '') : '',
                proceedId: b ? (b.id || '') : '',
                proceedOnclick: b ? (b.getAttribute('onclick') || '') : '',
                proceedFormId: f ? (f.id || '') : '',
                proceedFormAction: f ? (f.getAttribute('action') || '') : '',
                proceedFormMethod: f ? (f.getAttribute('method') || '') : '',
...
def process_captcha_input(message):
    """Handle a user-entered RTPS CAPTCHA without guessing/bypassing it.

    The important fix here is target selection and state verification:
    - choose the visible Proceed control belonging to the CAPTCHA form;
    - click the normal UI control (never submit the form directly);
    - check for an actual JS alert immediately;
    - prove a same-tab DOM transition before calling anything a next page;
    - if the CAPTCHA form remains, report exactly that instead of pretending
      that an alert/transition happened.
    """
    chat_id = message.chat.id
    user_id = message.from_user.id
    if not user_is_allowed(user_id):
    ...
    try:
        # 1) Fill the visible CAPTCHA input supplied by the user.
        captcha_input_elem = fill_visible_captcha_input(driver, captcha_text, timeout=20)
        send_step_screenshot(
            driver,
            chat_id,
            "FINAL_CAPTCHA_CODE_FILLED_FULL_SCREEN" if is_final_captcha else "CAPTCHA_CODE_FILLED_FULL_SCREEN"
        )

        pre_url = driver.current_url
        pre_handles = set(driver.window_handles)
        pre_handle = driver.current_window_handle
        pre_sig = get_rtps_page_signature(driver)

        # 2) Find the Proceed button associated with this CAPTCHA form.
        proceed_elem, proceed_info = _probe_captcha_proceed_candidates(
            driver, captcha_input_elem
        )
        if proceed_elem is None:
            _reset_processing()
            send_step_screenshot(driver, chat_id, "CAPTCHA_PROCEED_BUTTON_NOT_FOUND")
            bot.send_message(
                chat_id,
                "❌ CAPTCHA page पर सही Proceed button नहीं मिला।\n"
                "📸 Diagnostic screenshot भेज दिया गया है।\n"
                "⚠️ कोई दूसरा submit button अपने-आप नहीं दबाया गया।"
            )
            bot.set_state(user_id, RTPSState.captcha_input, chat_id)
            return

        # 3) Normal Selenium UI click. Do not call form.submit() or inject a
        # server request; RTPS's own client-side validation must run.
        try:
            driver.execute_script("arguments[0].scrollIntoView({block:'center'});", proceed_elem)
            time.sleep(0.3)
            proceed_elem.click()
        except Exception as click_err:
            logging.warning("Normal Proceed click failed, retrying same UI element: %r", click_err)
            try:
                driver.execute_script("arguments[0].click();", proceed_elem)
            except Exception as js_click_err:
                _reset_processing()
                logging.exception("RTPS Proceed click failed")
                send_error_screenshot(chat_id, driver, "❌ Proceed Click Error", js_click_err)
                bot.send_message(chat_id, f"❌ RTPS Proceed button click नहीं हुआ:\n{js_click_err}")
                bot.set_state(user_id, RTPSState.captcha_input, chat_id)
                return
        ...
        # Required screenshot immediately after the click, before any alert OK.
        send_step_screenshot(
            driver,
            chat_id,
        ...
        # 4) Short, deterministic alert poll. The old code waited 45s and then
        # later described the flow as alert-OK even when no alert existed.
        alert_handled = False
        alert_text = ""
        alert_kind = "NONE"
        try:
            WebDriverWait(driver, 5, poll_frequency=0.2).until(EC.alert_is_present())
            alert = driver.switch_to.alert
            alert_text = (alert.text or "").strip()
            lower = alert_text.casefold()

            captcha_error_markers = (
                "invalid captcha", "wrong captcha", "incorrect captcha",
                "captcha is invalid", "captcha invalid", "invalid capcha",
                "wrong capcha", "incorrect capcha", "अमान्य कैप्चा", "गलत कैप्चा",
                "कैप्चा गलत",
            )
            if any(marker in lower or marker in alert_text for marker in captcha_error_markers):
                alert_kind = "CAPTCHA_ERROR"
            elif any(marker in lower or marker in alert_text for marker in (
                "शुद्धता की जाँच", "शुद्धता की जांच", "mobile no.", "mobile no",
                "email की शुद्धता", "email की शुद्धता की जाँच", "email की शुद्धता की जांच",
                "sms/email", "सूचना प्राप्त"
            )):
                alert_kind = "FORM_VALIDATION"
            elif (("मोबाइल" in alert_text or "mobile" in lower) and
                  ("email" in lower or "ईमेल" in alert_text)):
                alert_kind = "VERIFIED"
            elif any(k in lower for k in ("verified", "success", "invalid otp", "incorrect otp", "wrong otp")):
                alert_kind = "OTP"
            else:
                alert_kind = "OTHER"

            # Screenshot before OK was already sent above. Now accept the
            # actual browser alert, if one really exists.
            try:
                alert.accept()
            finally:
                alert_handled = True
            ...
            logging.info(
                "RTPS real JS alert detected: kind=%s text=%s",
                alert_kind, mask_sensitive_data(alert_text)
            )
            session["last_activity"] = time.time()
            send_step_screenshot(
                driver,
                chat_id,
                "FINAL_CAPTCHA_AFTER_ALERT_OK" if is_final_captcha else "AFTER_CAPTCHA_ALERT_OK"
            )
        except TimeoutException:
            # No alert appeared. This is a valid outcome; do NOT claim OK.
            logging.info("RTPS Proceed produced no JS alert within 5s.")
        except Exception as alert_err:
            logging.debug("RTPS alert probe error: %r", alert_err)

        # 5) Explicit CAPTCHA error is handled immediately. Never infer it from
        # the mere presence of a CAPTCHA input.
        if alert_handled and alert_kind == "CAPTCHA_ERROR":
            session["captcha_attempts"] += 1
            _reset_processing()
            send_step_screenshot(
                driver, chat_id,
                "FINAL_CAPTCHA_REJECTED_FULL_SCREEN" if is_final_captcha else "CAPTCHA_REJECTED_FULL_SCREEN"
            )
            bot.send_message(
                chat_id,
                "❌ RTPS ने alert में CAPTCHA को गलत बताया है।\n"
                f"ℹ️ Alert: {mask_sensitive_data(alert_text)[:500]}\n"
                "🧩 कृपया नया CAPTCHA दर्ज करें।"
            )
            if session["captcha_attempts"] >= MAX_CAPTCHA_ATTEMPTS:
                bot.send_message(chat_id, "❌ अधिकतम CAPTCHA प्रयास समाप्त हुए।")
                finish_and_cleanup_session(chat_id, session)
                return
            try:
                p = os.path.join(user_dir, f"captcha_rejected_{int(time.time()*1000)}.png")
                if capture_captcha_image(driver, get_captcha_image_xpaths(), p):
                    with open(p, "rb") as fh:
                        bot.send_photo(chat_id, fh, caption="🧩 नया CAPTCHA दर्ज करें।", reply_markup=captcha_inline_keyboard())
                if os.path.exists(p):
                    os.remove(p)
            except Exception as e:
                logging.debug("CAPTCHA rejected image send failed: %r", e)
            bot.set_state(user_id, RTPSState.captcha_input, chat_id)
            return

        # 6) Prove same-tab DOM replacement. RTPS can keep the same URL, so URL
        # equality is irrelevant. The old CAPTCHA input becoming stale/gone is
        # the primary transition signal.
        transitioned = False
        try:
            WebDriverWait(driver, 20, poll_frequency=0.25).until(
                EC.staleness_of(captcha_input_elem)
            )
            transitioned = True
            logging.info("✅ Same-tab transition proven: old CAPTCHA element became stale.")
        except TimeoutException:
            # Secondary check only. A real state transition must also expose a
            # post-Proceed state; signature change alone is not enough.
            logging.info("Old CAPTCHA element stayed attached after Proceed; checking actual RTPS state.")
        except StaleElementReferenceException:
            transitioned = True

        # 7) Check the actual next state. Keep this short so a broken/no-op
        # Proceed does not leave the user waiting for several minutes.
        step = detect_post_proceed_state(driver, previous_url=pre_url, timeout=10)

        # A stale old CAPTCHA plus an actual known state is a strong transition.
        if step in ("MOBILE_OTP", "EMAIL_OTP", "ANNEXURE", "SUCCESS"):
            transitioned = True

        # Same URL is expected; only the actual controls/state decide success.
        if step == "ANNEXURE":
            _reset_processing()
            send_step_screenshot(
                driver, chat_id,
                "FINAL_NEXT_PAGE_AFTER_CAPTCHA_ALERT" if is_final_captcha else "NEXT_PAGE_AFTER_CAPTCHA_ALERT"
            )
            bot.send_message(
                chat_id,
                "📎 Actual RTPS Annexure page detect हो गया।\n"
                "➡️ अब Attach Annexure/document वाला step शुरू हो रहा है..."
            )
            execute_final_submission_internal(chat_id, session, "")
            return

        if step in ("MOBILE_OTP", "EMAIL_OTP", "SUCCESS"):
            _reset_processing()
            send_step_screenshot(driver, chat_id, "NEXT_PAGE_AFTER_CAPTCHA_ALERT")
            bot.send_message(chat_id, f"✅ RTPS next state detected: {step}")
            route_to_next_step(bot, chat_id, user_id, driver, user_dir)
            return

        # If the old CAPTCHA form is still visibly active, we have definitive
        # evidence that no page transition happened. This is the key fix for the
        # user's current symptom: never call this alert-OK or next-page.
        if _captcha_page_still_active(driver):
            _reset_processing()
            snap = _rtps_state_snapshot(driver)
            logging.warning("❌ CAPTCHA form still active after Proceed: %s", snap)
            send_step_screenshot(driver, chat_id, "CAPTCHA_PROCEED_NO_STATE_CHANGE")
            bot.send_message(
                chat_id,
                "⚠️ Proceed पर click हुआ, लेकिन RTPS ने page transition नहीं किया।\n"
                f"🔔 JS alert मिला: {'हाँ' if alert_handled else 'नहीं'}\n"
                f"☑️ Alert पर OK किया गया: {'हाँ' if alert_handled else 'नहीं'}\n"
                f"🔄 Same-tab transition: {'हाँ' if transitioned else 'नहीं'}\n"
                "📸 Current CAPTCHA page का diagnostic screenshot भेज दिया गया है।\n"
                "❗ इसे CAPTCHA गलत नहीं माना गया है और कोई नया CAPTCHA अपने-आप नहीं भरा गया।\n"
                "🔎 अब browser में दिख रहे Proceed/validation behavior की जांच करनी होगी।"
            )
            bot.set_state(user_id, RTPSState.captcha_input, chat_id)
            return

        # No CAPTCHA and no known next state: give one concise diagnostic rather
        # than waiting another minute or claiming success.
        _reset_processing()
        send_step_screenshot(driver, chat_id, "CAPTCHA_PROCEED_UNKNOWN_STATE")
        bot.send_message(
            chat_id,
            "⚠️ Proceed के बाद RTPS का known next state नहीं मिला।\n"
            f"🔔 JS alert: {'हाँ' if alert_handled else 'नहीं'} | "
            f"🔄 Same-tab DOM transition: {'हाँ' if transitioned else 'नहीं'}\n"
            "📸 Diagnostic screenshot भेज दिया गया है।"
        )
        bot.set_state(user_id, RTPSState.captcha_input, chat_id)
