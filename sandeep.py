...
def get_aadhaar_otp_xpaths():
    """RTPS Aadhaar OTP modal के लिए robust selectors."""
    return [
...
def get_aadhaar_validate_xpaths():
    """Aadhaar Number verification modal के visible Validate button selectors."""
    return [
...
def process_service_type(message):
    ...
    service = message.text.strip().upper()
    if service not in ["RESIDENCE", "CASTE", "INCOME"]:
        bot.reply_to(
            message,
            "कृपया केवल RESIDENCE, CASTE या INCOME में से ही विकल्प चुनें।"
        )
        return

    # Gemini flow: JSON manually भरवाने के बजाय पहले document लें।
    # Document मिलने पर process_document_upload() Gemini से JSON बनाएगा,
    # फिर user review/paste के लिए RTPSState.user_details पर भेजेगा।
    with bot.retrieve_data(user_id, chat_id) as data:
        data["service_type"] = service
        data["gemini_review_pending"] = False
        data.pop("gemini_data", None)
        data.pop("doc_path", None)

    bot.send_message(
        chat_id,
        f"✅ सर्विस '{service}' चुनी गई।\n\n"
        "🤖 Gemini वाला document-to-JSON flow चालू है।\n"
        "अब अपना Aadhaar/पहचान वाला document (PDF/JPG/PNG) भेजें।\n\n"
        "📄 Document मिलने के बाद Gemini उससे RTPS JSON तैयार करेगा।\n"
        "फिर bot आपको JSON दिखाएगा और आप उसे verify करके paste करेंगे।",
...
def process_captcha_input(message):
    ...
    try:
        ...
        # ------------------------------------------------------------
        # 4) If this was the CAPTCHA AFTER Aadhaar verification,
        #    successful Proceed means continue to final submission.
        #    If CAPTCHA is still visible, the answer was rejected.
        # ------------------------------------------------------------
        step = detect_current_verification_step(driver, timeout=6)
        ...
        if is_final_captcha:
            ...
            if step == "AADHAAR_OTP":
                ...
                bot.send_message(
                    chat_id,
                    "🔐 RTPS ने फिर से Aadhaar OTP माँगा है। नया OTP Telegram में भेजें।"
                )
                bot.set_state(user_id, RTPSState.aadhaar_otp_input, chat_id)
                return
                    ...
                    with open(current_captcha_path, "rb") as c_img:
                        bot.send_photo(
                            ...
                            caption=(
                                ...
                                "🧩 यह वही CAPTCHA है जो Proceed के बाद page पर दिख रहा है।\n"
