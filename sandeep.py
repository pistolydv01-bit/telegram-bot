# ===================================================================
# Python Telegram Bot Source Code (Updated with Step-by-Step Screenshots)
# ===================================================================
...
# ===================================================================
# Common Screenshot Helper Function (As Suggested)
# ===================================================================

def send_step_screenshot(driver, chat_id, step_name):
    if not chat_id or not driver:
        return
    try:
        ...
        path = os.path.join(
            user_dir,
            f"{int(time.time())}_{safe_name}.png"
        )
        ...
        logging.info("Screenshot sent: %s", step_name)
...
def debug_screenshot(driver, chat_id, name):
    send_step_screenshot(driver, chat_id, f"DEBUG_{name}")
...
def fill_initial_rtps_form(driver, user_data, service_type, photo_path, chat_id=None, user_id=None):
    ...
    if chat_id:  
        send_step_screenshot(driver, chat_id, "RTPS_HOMEPAGE")
    ...
    time.sleep(1)  
    send_step_screenshot(driver, chat_id, "GENDER_SELECTED")  
    ...
    send_step_screenshot(driver, chat_id, "APPLICANT_NAME_FILLED")  
    ...
    send_step_screenshot(driver, chat_id, "FATHER_NAME_FILLED")  
    ...
    if user_data.get("mother_name"):      
        try:      
            ...
            send_step_screenshot(driver, chat_id, "MOTHER_NAME_FILLED")      
        except Exception as e:      
    ...
    if user_data.get("email"):      
        try:      
            ...
            send_step_screenshot(driver, chat_id, "EMAIL_CLICKED")
            time.sleep(1.5)  

            if detect_mobile_otp_popup(driver, timeout=5):  
                send_step_screenshot(driver, chat_id, "MOBILE_OTP_POPUP")
                with session_lock:  
...
def continue_rtps_form_after_mobile_otp(driver, user_data, service_type, photo_path, chat_id=None, user_id=None):
    try:
        ...
        if user_data.get("email"):
            try:
                find_and_interact(driver, [
                    "//input[@id='email' or contains(@id, 'email') or contains(@name, 'email')]"
                ], "type", user_data["email"], chat_id=chat_id)
                send_step_screenshot(driver, chat_id, "EMAIL_FILLED")
            except Exception as e:
                logging.warning(f"Email type warning after OTP: {e}")
        ...
        if user_data.get("ward_no"):      
            try:      
                ...
                send_step_screenshot(driver, chat_id, "WARD_NO_FILLED")
            except Exception as e:      
        ...
        if user_data.get("village"):      
            try:      
                ...
                send_step_screenshot(driver, chat_id, "VILLAGE_FILLED")
            except Exception as e:      
        ...
        if user_data.get("post_office"):      
            try:      
                ...
                send_step_screenshot(driver, chat_id, "POST_OFFICE_FILLED")
            except Exception as e:      
        ...
        if user_data.get("police_station"):      
            try:      
                ...
                send_step_screenshot(driver, chat_id, "POLICE_STATION_SELECTED")
            except Exception as e:      
        ...
        if service_type == "RESIDENCE":      
            ...
            try:      
                ...
                send_step_screenshot(driver, chat_id, "RESIDENCE_TYPE_SELECTED")
            except Exception as e:      
            ...
            send_step_screenshot(driver, chat_id, "CASTE_DETAILS_FILLED")
        elif service_type == "INCOME":      
            select_dropdown_robustly(driver, By.XPATH, "//select[@id='profession' or contains(@name, 'profession')]", user_data.get("profession"), chat_id=chat_id)      
            find_and_interact(driver, ["//input[@id='annual_income' or contains(@name, 'income')]"], "type", str(user_data["annual_income"]), chat_id=chat_id)      
            send_step_screenshot(driver, chat_id, "INCOME_DETAILS_FILLED")
    except Exception as e:  
        logging.error(f"Error in continue_rtps_form_after_mobile_otp: {e}")  
        raise e
...
def process_mobile_otp_input(message):
    ...
    try:      
        ...
        send_step_screenshot(driver, chat_id, "MOBILE_OTP_ENTERED")
        ...
        try:
            ...
            logging.info("No OTP alert appeared")
        ...
        if not otp_verified_successfully and dom_result == "INVALID":
            otp_result = "INVALID"
        elif otp_verified_successfully:
            otp_result = "SUCCESS"
        else:
        ...
        send_step_screenshot(driver, chat_id, "NEW_PAGE_AFTER_OTP_CLOSED")
