...
# Dynamic ALLOWED_USER_IDS from .env
...
# -------------------------------------------------------------------    
# Error Screenshot Helper Function
# -------------------------------------------------------------------    
def send_error_screenshot(chat_id, driver, error_message="Automation Error"):
    try:
        if not driver:
            return
        ...
        screenshot_path = os.path.join(user_dir, f"ERROR_{int(time.time())}.png")
...
def find_and_interact(driver, xpaths, action_type="click", text_value=None, timeout=20):    
    ...
    try:
        ...
        # Active session से chat_id ढूंढ कर Telegram पर screenshot भेजना
        error_chat_id = None
        for cid, session in active_user_sessions.items():
            if session.get("driver") == driver:
                error_chat_id = cid
                break
                
        if error_chat_id:
            send_error_screenshot(error_chat_id, driver, "❌ Selenium XPath/Element Error")
            
    except Exception as screenshot_error:
        logging.error(f"Debug screenshot error: {screenshot_error}")
...
# -------------------------------------------------------------------    
# RTPS Form Filling Logic (Updated with Robust GAD Navigation)
# -------------------------------------------------------------------    
def fill_initial_rtps_form(driver, user_data, service_type, photo_path, chat_id=None):    
    ...
    if chat_id:
        try:
            ...
            with open(ss_path, 'rb') as ss_file:
                bot.send_photo(chat_id, ss_file, caption="🌐 RTPS Website open हो gayi hai.")
            logging.info("Screenshot successfully sent to Telegram.")
        except Exception as ss_err:
            logging.warning(f"Screenshot error: {ss_err}")

    # Robust Navigation: General Administration Department
    logging.info("Clicking General Administration Department Accordion...")
            ...
            for elem in elems:
                if elem.is_displayed():
                    ...
                    try:
                        ...
                        driver.execute_script("arguments[0].click();", elem)
                    gad_clicked = True
                    logging.info(f"✅ GAD clicked using XPath: {xp}")
                    break
            if gad_clicked:
                break
        except Exception as e:
            logging.warning(f"GAD XPath failed: {xp} -> {e}")

    if not gad_clicked:
        try:
            result = driver.execute_script("""
                const targetText = 'सामान्य प्रशासन विभाग';
                const all = Array.from(document.querySelectorAll('a, li, span, div, td, label'));
            ...
            if result:
                gad_clicked = True
                logging.info("✅ GAD clicked using JavaScript fallback")
        except Exception as e:
            logging.error(f"GAD JS fallback error: {e}")

    if not gad_clicked:
        try:
            debug_path = os.path.join(get_user_dir(chat_id) if chat_id else os.getcwd(), "gad_error.png")
            driver.save_screenshot(debug_path)
            if chat_id:
                send_error_screenshot(chat_id, driver, "❌ RTPS homepage पर 'सामान्य प्रशासन विभाग' menu नहीं मिला।")
        except Exception:
            pass
        raise NoSuchElementException("❌ RTPS homepage पर 'सामान्य प्रशासन विभाग' menu नहीं मिला।")
    ...
    service_links = {
        "RESIDENCE": [
            ...
            "//a[contains(normalize-space(.), 'आवासीय प्रमाण पत्र')]",
            "//*[contains(normalize-space(.), 'आवासीय प्रमाण-पत्र')]",
            "//*[contains(normalize-space(.), 'आवासीय प्रमाण पत्र')]",
        ...
        "CASTE": [
            ...
            "//a[contains(normalize-space(.), 'जाति प्रमाण पत्र')]",
            "//*[contains(normalize-space(.), 'जाति प्रमाण-पत्र')]",
            "//*[contains(normalize-space(.), 'जाति प्रमाण पत्र')]",
        ...
        "INCOME": [
            ...
            "//a[contains(normalize-space(.), 'आय प्रमाण पत्र')]",
            "//*[contains(normalize-space(.), 'आय प्रमाण-पत्र')]",
            "//*[contains(normalize-space(.), 'आय प्रमाण पत्र')]",
            "//a[contains(@href, 'income')]"
        ]
    }
    find_and_interact(driver, service_links[service_type], action_type="click", timeout=15)
    time.sleep(2)
    ...
    find_and_interact(driver, block_xpaths, action_type="click", timeout=15)
    time.sleep(5)
...
def process_document_upload(message):    
    ...
    try:    
        ...
        # Error Screenshot Hook Added
        active_driver = None
        with session_lock:
            s_obj = active_user_sessions.get(chat_id)
            if s_obj:
                active_driver = s_obj.get("driver")
        send_error_screenshot(chat_id, active_driver, "Automation Critical Error")
...
def process_mobile_otp_input(message):    
    ...
    try:    
        ...
        send_error_screenshot(chat_id, driver, "Mobile OTP Error")
...
def process_email_otp_input(message):    
    ...
    try:    
        ...
        send_error_screenshot(chat_id, driver, "Email OTP Error")
...
def process_captcha_input(message):    
    ...
    try:    
        ...
        send_error_screenshot(chat_id, driver, "Captcha Error")
...
def execute_final_submission_internal(chat_id, session, otp_code):    
    ...
    try:    
        ...
        send_error_screenshot(chat_id, driver, "Final Submission Error")
    finally:    
        if should_cleanup:    
            finish_and_cleanup_session(chat_id, session)    
        if user_locks[chat_id].locked():    
