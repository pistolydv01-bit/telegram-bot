...
# ============================================================
# CONTACT FIELDS DEFERRED UNTIL FINAL FORM STAGE
# ============================================================
# Existing Email/Mobile code is kept intact below, but guarded
# so it does not trigger mobile verification before income fields.
DEFER_CONTACT_FIELDS = True
...
def fill_initial_rtps_form(driver, user_data, service_type, photo_path, chat_id=None, user_id=None):
    ...
    # ============================================================
    # EXACT ID INTEGRATION FOR PROFESSION, PURPOSE, INCOME & CHECKBOX
    # ============================================================
    try:
        # 1. Profession Dropdown (ID = 17207)
        profession_val = user_data.get("profession")
        if profession_val:
            logging.info(f"📍 Selecting Profession = {profession_val} (ID: 17207)")
            prof_elem = WebDriverWait(driver, 20).until(
                EC.presence_of_element_located((By.ID, "17207"))
            )
            Select(prof_elem).select_by_visible_text(str(profession_val).strip())
            driver.execute_script("""
                arguments[0].dispatchEvent(new Event('change', {bubbles: true}));
            """, prof_elem)
            time.sleep(1)

        # 2. Purpose of Application (ID = 17172)
        purpose_val = user_data.get("purpose")
        if purpose_val:
            logging.info(f"📍 Filling Purpose = {purpose_val} (ID: 17172)")
            find_and_interact(driver, ["//input[@id='17172']"], "type", purpose_val, timeout=20, chat_id=chat_id)

        # 3. Govt Service Income (ID = 17168)
        gov_income = user_data.get("income_govt_service", "00")
        find_and_interact(driver, ["//input[@id='17168']"], "type", gov_income, timeout=20, chat_id=chat_id)

        # 4. Agriculture Income (ID = 17171)
        agri_income = user_data.get("income_agriculture", "00")
        find_and_interact(driver, ["//input[@id='17171']"], "type", agri_income, timeout=20, chat_id=chat_id)

        # 5. Business Income (ID = 17169)
        biz_income = user_data.get("income_business", "00")
        find_and_interact(driver, ["//input[@id='17169']"], "type", biz_income, timeout=20, chat_id=chat_id)

        # 6. Other Sources Income (ID = 17170)
        other_income = user_data.get("income_other_sources", "00")
        find_and_interact(driver, ["//input[@id='17170']"], "type", other_income, timeout=20, chat_id=chat_id)

        # 7. Total Annual Income Verification (ID = 17173 - Readonly)
        try:
            total_income = driver.find_element(By.ID, "17173").get_attribute("value")
            logging.info(f"✅ Total Annual Income (ID: 17173) = {total_income}")
        except Exception as e:
            logging.warning(f"Total income verification warning: {e}")

        # 8. अन्यान्य अभिलेख Checkbox (ID = 94045_3)
        other_records = user_data.get("other_records", True)
        if other_records:
            chk = WebDriverWait(driver, 20).until(
                EC.element_to_be_clickable((By.ID, "94045_3"))
            )
            if not chk.is_selected():
                driver.execute_script("arguments[0].click();", chk)
            logging.info("✅ 'अन्यान्य अभिलेख' checkbox selected (ID: 94045_3)")

        # Fallback for category, caste, residence_type if provided
        ...
        logging.warning(f"Service/Income fields warning: {e}")
    ...
    # ============================================================
    if user_data.get("email") and not DEFER_CONTACT_FIELDS:
        try:
    ...
    # ============================================================
    # FINAL CONTACT FIELDS - EMAIL + MOBILE BEFORE CAPTCHA
    # ============================================================
    if DEFER_CONTACT_FIELDS:
        # Email is intentionally filled here, after all other form fields.
        if user_data.get("email"):
            try:
                ...
                logging.info("✅ Email filled at final stage before CAPTCHA")
            except Exception as e:
                logging.warning(f"⚠️ Final Email filling warning: {e}")

        # Mobile is intentionally the LAST form field because typing/ENTER
        # can open the mobile verification consent/OTP popup.
        try:
            mobile_xpaths = [
                "//input[contains(translate(@id,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'mobile')]",
                "//input[contains(translate(@name,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'mobile')]",
            ...
            logging.info("✅ Mobile number filled at FINAL stage before CAPTCHA")
            time.sleep(1)

            # Trigger the portal's mobile verification.
            try:
                mobile_element.send_keys(Keys.ENTER)
            except Exception:
                driver.execute_script("""
            ...
            # Handle the consent popup shown by the portal before OTP.
            agree_xpaths = [
                ...
                "//*[self::a or self::span][normalize-space()='Agree']"
            ]
            try:
                find_and_interact(
                    driver,
                ...
                # The popup may already have been accepted or may not be shown.
                logging.info(f"ℹ️ Agree popup not found/already handled: {e}")

            # Wait for OTP popup after consent.
            if detect_mobile_otp_popup(driver, timeout=10):
            ...
            logging.warning("⚠️ Final mobile OTP popup detected नहीं हुआ")

        except Exception as e:
            logging.warning(f"⚠️ Final Mobile number / OTP trigger warning: {e}")
...
# ============================================================
# CAPTCHA REFRESH - Telegram Button Support
# ============================================================
def captcha_refresh_keyboard():
    markup = telebot.types.InlineKeyboardMarkup()
    markup.add(
        telebot.types.InlineKeyboardButton(
            "🔄 Refresh CAPTCHA",
            callback_data="rtps_refresh_captcha"
        )
    )
    return markup
...
def refresh_captcha_image(driver, timeout=10):
    """Try the portal's CAPTCHA refresh control using several safe selectors."""
    driver.switch_to.default_content()

    refresh_xpaths = [
        "//button[contains(translate(@id,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'captcha') and (contains(translate(@id,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'refresh') or contains(translate(@id,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'reload'))]",
        "//a[contains(translate(@id,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'captcha') and (contains(translate(@id,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'refresh') or contains(translate(@id,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'reload'))]",
        "//*[self::button or self::a or self::input][contains(translate(@title,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'refresh') and (contains(translate(@id,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'captcha') or contains(translate(@class,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'captcha'))]",
        "//*[self::button or self::a or self::img][contains(translate(@alt,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'refresh') and contains(translate(@src,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'captcha')]",
        "//*[self::button or self::a or self::img][contains(translate(@onclick,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'captcha') and (contains(translate(@onclick,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'refresh') or contains(translate(@onclick,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'reload'))]",
        "//img[contains(translate(@id,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'captcha')]/following-sibling::*[self::a or self::button][1]",
        "//img[contains(translate(@id,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'captcha')]/parent::*[self::a or self::button]"
    ]
            ...
            for elem in elems:
                if elem.is_displayed() and elem.is_enabled():
                    ...
                    try:
                        ...
                        driver.execute_script("arguments[0].click();", elem)
                    time.sleep(1.5)
                    logging.info(f"✅ CAPTCHA refreshed using XPath: {xp}")
                    return True
        except Exception as e:
            logging.debug(f"CAPTCHA refresh XPath failed: {xp} | {e}")

    # JS fallback for portals that bind the refresh action through onclick.
    try:
        result = driver.execute_script("""
            const nodes = Array.from(document.querySelectorAll('*'));
            const el = nodes.find(e => {
                if (!e || !e.offsetParent) return false;
                const t = ((e.id||'')+' '+(e.className||'')+' '+(e.title||'')+' '+(e.alt||'')+' '+(e.getAttribute('onclick')||'')).toLowerCase();
                return t.includes('captcha') && (t.includes('refresh') || t.includes('reload'));
            });
        ...
        if result:
            time.sleep(1.5)
            logging.info("✅ CAPTCHA refreshed using JavaScript fallback")
            return True
    except Exception as e:
        logging.debug(f"CAPTCHA refresh JS fallback failed: {e}")

    return False


def send_captcha_refresh_button(bot, chat_id):
    """Add a Telegram refresh button without removing the existing CAPTCHA message."""
    try:
        bot.send_message(
            chat_id,
            "🔄 CAPTCHA साफ/समझ में नहीं आ रहा है? नीचे Refresh दबाकर नया CAPTCHA मंगाएं।",
            reply_markup=captcha_refresh_keyboard()
        )
    except Exception as e:
        logging.warning(f"CAPTCHA refresh button send error: {e}")
    ...
    for _ in range(max_retries):
        ...
        if step == "MOBILE_OTP":      
            ...
            if capture_captcha_image(driver, captcha_xpaths, captcha_img_path):      
                ...
                send_captcha_refresh_button(bot, chat_id)
...
@bot.callback_query_handler(func=lambda call: call.data == "rtps_refresh_captcha")
def refresh_captcha_callback(call):
    chat_id = call.message.chat.id
    user_id = call.from_user.id

    if not user_is_allowed(user_id):
        try:
            bot.answer_callback_query(call.id, "❌ अनुमति नहीं है।", show_alert=True)
        except Exception:
            pass
        return
    ...
    with session_lock:
        ...
        if not session:
            try:
                bot.answer_callback_query(call.id, "⚠️ Session समाप्त हो गया है।", show_alert=True)
            except Exception:
                pass
            return
        session["is_processing"] = True
    ...
    try:
        ...
        # Make sure we are still on the CAPTCHA stage.
        step = detect_current_verification_step(driver, timeout=5)
        if step != "CAPTCHA":
            session["is_processing"] = False
            bot.send_message(
                chat_id,
                f"⚠️ अभी CAPTCHA stage नहीं है (current step: {step})."
            )
            return

        refreshed = refresh_captcha_image(driver, timeout=10)
        if not refreshed:
            session["is_processing"] = False
            bot.send_message(
                chat_id,
                "❌ CAPTCHA refresh button portal पर नहीं मिला।"
            )
            return

        # Capture the newly refreshed CAPTCHA and send it to Telegram.
        captcha_img_path = os.path.join(
            user_dir,
            f"captcha_refresh_{int(time.time() * 1000)}.png"
        )
        captcha_xpaths = [
            "//img[contains(@id, 'captcha') or contains(@src, 'captcha') or contains(@id, 'imgCaptcha')]"
        ]
        ...
        if capture_captcha_image(driver, captcha_xpaths, captcha_img_path):
            with open(captcha_img_path, "rb") as c_img:
                bot.send_photo(
                    chat_id,
                    c_img,
                    caption="🔄 नया CAPTCHA: इसे दर्ज करें:"
                )
            send_captcha_refresh_button(bot, chat_id)
            bot.set_state(user_id, RTPSState.captcha_input, chat_id)
            logging.info("✅ New CAPTCHA screenshot sent after Telegram refresh")
        else:
            bot.send_message(chat_id, "⚠️ नया CAPTCHA screenshot नहीं लिया जा सका।")
        ...
        session["is_processing"] = False
        session["last_activity"] = time.time()
        logging.error(f"CAPTCHA refresh callback error: {e}")
        bot.send_message(chat_id, f"❌ CAPTCHA refresh में त्रुटि:\n{e}")
