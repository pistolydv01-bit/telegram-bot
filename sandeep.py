# ===================================================================
# Python Telegram Bot Source Code (Updated with Safe Steps & Next Step Flow)
# ===================================================================
...
def select_dropdown_robustly(driver, element_by, locator_value, target_text, timeout=20, min_options=1, chat_id=None):
    ...
    raise NoSuchElementException(f"Dropdown option '{target_text}' not found for {locator_value}")
    ...
    while time.time() - start < timeout:
        try:
            ...
            error_xpaths = [
                ...
                "//*[contains(normalize-space(.), 'अमान्य OTP')]",
                "//*[contains(normalize-space(.), 'गलत OTP')]",
...
def fill_initial_rtps_form(driver, user_data, service_type, photo_path, chat_id=None, user_id=None):
    ...
    # Address & Location Fields filling in logical order
    try:
        select_dropdown_robustly(driver, By.XPATH, "//select[contains(translate(@id,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'state')]", "बिहार", timeout=8, chat_id=chat_id)
        select_dropdown_robustly(driver, By.XPATH, "//select[contains(translate(@id,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'district')]", user_data.get("district"), timeout=8, chat_id=chat_id)
        select_dropdown_robustly(driver, By.XPATH, "//select[contains(translate(@id,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'sub')]", user_data.get("sub_division"), timeout=8, chat_id=chat_id)
        select_dropdown_robustly(driver, By.XPATH, "//select[contains(translate(@id,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'block') or contains(translate(@id,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'circle')]", user_data.get("block"), timeout=8, chat_id=chat_id)
        
        if user_data.get("local_body_type"):
            select_dropdown_robustly(driver, By.XPATH, "//select[contains(translate(@id,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'localbody') or contains(translate(@id,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'bodytype')]", user_data.get("local_body_type"), timeout=8, chat_id=chat_id)
        
        if user_data.get("panchayat"):
            select_dropdown_robustly(driver, By.XPATH, "//select[contains(translate(@id,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'panchayat')]", user_data.get("panchayat"), timeout=8, chat_id=chat_id)
        
        if user_data.get("village"):
            find_and_interact(driver, ["//input[contains(translate(@id,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'village') or contains(translate(@name,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'village')]"], "type", user_data.get("village"), chat_id=chat_id)
        
        if user_data.get("ward_no"):
            find_and_interact(driver, ["//input[contains(translate(@id,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'ward') or contains(translate(@name,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'ward')]"], "type", user_data.get("ward_no"), chat_id=chat_id)
        
        if user_data.get("post_office"):
            find_and_interact(driver, ["//input[contains(translate(@id,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'postoffice') or contains(translate(@id,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'po')]"], "type", user_data.get("post_office"), chat_id=chat_id)
            
        if user_data.get("police_station"):
            find_and_interact(driver, ["//input[contains(translate(@id,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'policestation') or contains(translate(@id,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'ps')]"], "type", user_data.get("police_station"), chat_id=chat_id)

        if user_data.get("pin_code"):
            find_and_interact(driver, ["//input[contains(translate(@id,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'pincode') or contains(translate(@id,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'pin')]"], "type", user_data.get("pin_code"), chat_id=chat_id)
    except Exception as e:
        logging.warning(f"Address fields filling warning: {e}")
    ...
    # 2. Aadhaar Number filling (Before Mobile Number)
    ...
    # 3. Mobile Number - FILLED AT THE VERY LAST of the main form
    try:
        find_and_interact(driver, [  
        ...
        debug_screenshot(driver, chat_id, "AFTER_MOBILE_LAST")  
    except Exception as e:
        logging.warning(f"Mobile number filling warning: {e}")
    ...
    if user_data.get("email"):      
        try:      
            ...
            find_and_interact(driver, email_xpaths, action_type="click", timeout=15, chat_id=chat_id)  
            time.sleep(1.5)  

            if detect_mobile_otp_popup(driver, timeout=5):  
                with session_lock:  
                    if chat_id in active_user_sessions:  
                        active_user_sessions[chat_id].update({"is_processing": False, "waiting_for": "MOBILE_OTP"})  
                  
                bot.send_message(chat_id, "📱 RTPS ने आपके मोबाइल नंबर पर OTP भेजा है। कृपया OTP भेजें:")  
                if user_id:  
                    bot.set_state(user_id, RTPSState.mobile_otp_input, chat_id)  
                return "MOBILE_OTP"  
        except Exception as e:      
            logging.warning(f"Email click / Mobile OTP trigger warning: {e}")
...
def process_mobile_otp_input(message):
    ...
    try:      
        ...
        try:
            # Send step screenshot immediately after Mobile OTP verification is successful
...
def process_captcha_input(message):
    ...
    try:      
        ...
        # CAPTCHA के बाद 'Process' button click करवाना (Submit नहीं)
        process_btn_xpaths = [
...
def execute_final_submission_internal(chat_id, session, otp_code):
    ...
    try:      
        ...
        # Aadhaar OTP verification के बाद Final Submit और Document Upload
        try:      
        ...
        try:      
            ...
            attach_btn = wait.until(EC.element_to_be_clickable((By.XPATH, attach_xpaths[0])))
