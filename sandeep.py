...
# ===================================================================
# UPDATED AJAX DROPDOWN FUNCTIONS (Label-Based & Dependent Options Wait)
# ===================================================================

def select_location_dropdown(driver, label_words, target_text, timeout=25):
    """
    RTPS dependent dropdown selector. State -> District -> Sub-Division -> Block -> Panchayat
    """
        ...
        try:
            ...
            for select_elem in selects:
                try:
                    ...
                    # Find the nearest useful container having only this select
                    context_text = driver.execute_script("""
                        const select = arguments[0];
                        let node = select;
                        for (let i = 0; i < 7 && node; i++) {
                            const text = (node.innerText || '').trim();
                            const selects = node.querySelectorAll ? node.querySelectorAll('select').length : 0;
                            if (
                                text &&
                                selects === 1 &&
                                text.length < 500
                    ...
                    context_text = " ".join((context_text or "").split()).lower()
                    
                    # This select must belong to requested field
                    if not any(str(word).strip().lower() in context_text for word in label_words):
                        continue
                    ...
                    if matched_option is None:
                        for option in options:
                            ...
                            if not option_text:
                                continue
                            if (target_text.lower() in option_text.lower() or option_text.lower() in target_text.lower()):
                                matched_option = option
                                break
                    
                    # AJAX options अभी नहीं आए
                    if matched_option is None:
                        continue
                    ...
                    # Select by option value where possible
                    option_value = matched_option.get_attribute("value")
                    ...
                    if (selected_text.lower() == target_text.lower() or target_text.lower() in selected_text.lower()):
                        logging.info(f"✅ Dropdown selected: {label_words[0]} -> {selected_text}")
                        return True
                except Exception as e:
                    logging.debug(f"Dropdown retry {label_words}: {e}")
        except Exception as e:
            logging.debug(f"Dropdown scan error: {e}")
            
        # AJAX option loading wait time
        time.sleep(0.5)
        
    logging.error(f"❌ Dropdown selection failed: {label_words} -> {target_text}")
    return False
...
def fill_initial_rtps_form(driver, user_data, service_type, photo_path, chat_id=None, user_id=None):
    ...
    # ============================================================ #
    # ADDRESS / LOCATION # State -> District -> Sub-Division -> Block
    # ============================================================ #
    logging.info("🏠 Starting Address & Location fields...")
    try:
        # STATE
        if not select_location_dropdown(driver, ["State", "राज्य"], "बिहार", timeout=25):
            raise Exception("State 'बिहार' select नहीं हुआ")
        time.sleep(1)

        # DISTRICT
        if user_data.get("district"):
            if not select_location_dropdown(driver, ["District", "जिला"], user_data["district"], timeout=25):
                raise Exception(f"District select नहीं हुआ: {user_data['district']}")
        time.sleep(1)

        # SUB-DIVISION
        if user_data.get("sub_division"):
            if not select_location_dropdown(driver, ["Sub-Division", "अनुमंडल"], user_data["sub_division"], timeout=25):
                raise Exception(f"Sub-Division select नहीं हुआ: {user_data['sub_division']}")
        time.sleep(1)

        # BLOCK
        if user_data.get("block"):
            if not select_location_dropdown(driver, ["Block", "प्रखंड"], user_data["block"], timeout=25):
                raise Exception(f"Block select नहीं हुआ: {user_data['block']}")
        time.sleep(1)

        # ============================================================ #
        # TYPE OF LOCAL BODY # PERMANENT = VILLAGE PANCHAYAT
        # ============================================================ #
        try:
            village_radio = None
            ...
            for label in labels:
                try:
                    ...
                    if for_id:
                        radios = driver.find_elements(By.ID, for_id)
                        if radios:
                            village_radio = radios[0]
                            break
                    # Case 2: radio inside label
                    radios = label.find_elements(By.XPATH, ".//input[@type='radio']")
                    if radios:
                        village_radio = radios[0]
                        break
                    # Case 3: radio immediately before label
                    radios = label.find_elements(By.XPATH, "./preceding-sibling::input[@type='radio'][1]")
                    if radios:
                        village_radio = radios[0]
                        break
                    # Case 4: radio immediately after label
                    radios = label.find_elements(By.XPATH, "./following-sibling::input[@type='radio'][1]")
                    if radios:
                        village_radio = radios[0]
                        break
                except Exception:
            ...
            if village_radio is None:
                raise Exception("Village Panchayat radio button नहीं मिला")
            ...
            logging.info("✅ Village Panchayat permanently selected.")
        except Exception as e:
            logging.error(f"❌ Village Panchayat selection failed: {e}")
            try:
                debug_screenshot(driver, chat_id, "VILLAGE_PANCHAYAT_ERROR")
            except Exception:
                pass
        ...
        if panchayat:
            if not select_location_dropdown(driver, ["Panchayat", "ग्राम पंचायत"], panchayat, timeout=25):
