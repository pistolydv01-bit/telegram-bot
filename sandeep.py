# ===================================================================
# Python Telegram Bot Source Code (Updated with Numeric ID Address Flow)
# ===================================================================
...
def fill_initial_rtps_form(driver, user_data, service_type, photo_path, chat_id=None, user_id=None):
    ...
    # ============================================================
    # RTPS ADDRESS - NUMERIC ID DEPENDENT DROPDOWN FLOW (EXACT REPLACEMENT)
    # ============================================================
    try:
        logging.info("🏠 Starting RTPS address selection with Numeric IDs...")

        def wait_dropdown_options(select_id, timeout=30):
        ...
        def select_by_text_match(select_id, target, timeout=30):
        ...
        select_by_text_match("17162", district, timeout=30)
        time.sleep(1)
        ...
        if subdivision:
            ...
            wait_dropdown_options("17159", timeout=30)
            select_by_text_match("17159", subdivision, timeout=30)
            time.sleep(1)
        ...
        if block:
            ...
            wait_dropdown_options("17163", timeout=30)
            select_by_text_match("17163", block, timeout=30)
            time.sleep(1)

        # 5. VILLAGE PANCHAYAT RADIO (id="75265_1")
        try:
            village_radio = WebDriverWait(driver, 20).until(
                EC.presence_of_element_located((By.ID, "75265_1"))
            )
            ...
            driver.execute_script("""
                ...
                arguments[0].dispatchEvent(new Event('change', {bubbles:true}));
            """, village_radio)
            logging.info("✅ Village Panchayat radio selected (75265_1)")
        except Exception as e:
            logging.warning(f"⚠️ Village Panchayat radio selection warning (non-fatal): {e}")

        # 6. GRAM PANCHAYAT (id="gpListID")
        panchayat = str(user_data.get("panchayat", "")).strip()
        if panchayat:
            try:
                wait_dropdown_options("gpListID", timeout=30)
                select_by_text_match("gpListID", panchayat, timeout=30)
                logging.info(f"✅ Gram Panchayat selected: {panchayat}")
            except Exception as e:
                logging.warning(f"⚠️ Gram Panchayat selection warning (non-fatal): {e}")
        ...
        if village:
            try:
                village_xpaths = [
                    "//input[contains(translate(@id,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'village')]",
                    "//input[contains(translate(@name,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'village')]"
                ]
                find_and_interact(driver, village_xpaths, "type", village, chat_id=chat_id)
            except Exception as e:
                logging.warning(f"⚠️ Village input warning: {e}")
        ...
        if ward:
            try:
                ward_xpaths = [
                    "//input[contains(translate(@id,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'ward')]",
                    "//input[contains(translate(@name,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'ward')]"
                ]
                find_and_interact(driver, ward_xpaths, "type", ward, chat_id=chat_id)
            except Exception as e:
                logging.warning(f"⚠️ Ward input warning: {e}")
        ...
        if post_office:
            try:
                post_xpaths = [
                    "//input[contains(translate(@id,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'postoffice')]",
                    "//input[contains(translate(@name,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'postoffice')]"
                ]
                find_and_interact(driver, post_xpaths, "type", post_office, chat_id=chat_id)
            except Exception as e:
                logging.warning(f"⚠️ Post Office input warning: {e}")
        ...
        if police_station:
            try:
                ps_xpaths = [
                    "//input[contains(translate(@id,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'policestation')]",
                    "//input[contains(translate(@name,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'policestation')]"
                ]
                find_and_interact(driver, ps_xpaths, "type", police_station, chat_id=chat_id)
            except Exception as e:
                logging.warning(f"⚠️ Police Station input warning: {e}")
        ...
        if pin_code:
            try:
                pin_xpaths = [
                    ...
                    "//input[contains(translate(@name,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'pincode')]"
                ]
                find_and_interact(driver, pin_xpaths, "type", pin_code, chat_id=chat_id)
            except Exception as e:
                logging.warning(f"⚠️ Pin Code input warning: {e}")
        ...
        # नॉन-क्रिटिकल एरर होने पर फ्लो न रुके, इसलिए यहाँ raise हटाकर वार्निंग दी गई है या आप चाहें तो रख सकते हैं। 
        # फिलहाल सुरक्षित रखने के लिए इसे क्रिटिकल एरर के रूप में हैंडल किया गया है।
        raise

    # Photo Upload in Main Form (Actual File Input id="17495" or generic)
