# ===================================================================
# Python Telegram Bot Source Code (Updated with AJAX Dropdown & Safe Steps)
# ===================================================================
...
# ===================================================================
# UPDATED AJAX DROPDOWN FUNCTIONS
# ===================================================================
...
def wait_for_dropdown_element(driver, element_by, locator_value, timeout=20):
    def _find_dropdown(d):
        try:
            elements = d.find_elements(element_by, locator_value)
            ...
            return False
    return WebDriverWait(driver, timeout, poll_frequency=0.3).until(_find_dropdown)

def wait_for_dropdown_target(driver, element_by, locator_value, target_text, timeout=25):
    target = _normalize_dropdown_text(target_text)
    if not target:
        return None
    ...
    while time.time() < end_time:
        try:
            element = wait_for_dropdown_element(driver, element_by, locator_value, timeout=3)
            if element.tag_name.lower() == "select":
                try:
                    select = Select(element)
                    options = select.options
                    for option in options:
                        text = _normalize_dropdown_text(option.text)
                        value = _normalize_dropdown_text(option.get_attribute("value"))
                        if text == target or value == target:
            ...
            logging.debug(f"Dropdown target wait retry: {e}")
            time.sleep(0.5)
    return None

def select_dropdown_robustly(driver, element_by, locator_value, target_text, timeout=25, min_options=2, chat_id=None):
    if not target_text:
        logging.warning(f"Dropdown target खाली है: {locator_value}")
        return False
        
    target_text = str(target_text).strip()
    logging.info(f"🔽 Dropdown selecting: {target_text} | {locator_value}")
    
    result = wait_for_dropdown_target(driver, element_by, locator_value, target_text, timeout=timeout)
    if result is None:
        logging.warning(f"❌ Target option नहीं मिला: '{target_text}'")
        try:
            element = driver.find_element(element_by, locator_value)
            if element.tag_name.lower() == "select":
                options = Select(element).options
                available = [option.text.strip() for option in options if option.text.strip()]
                logging.warning(f"Available options: {available[:50]}")
        except Exception as e:
            logging.debug(f"Options debug failed: {e}")
        return False
        
    element, matched_option = result
    ...
    try:
        select = Select(element)
        option_text = matched_option.text
        option_value = matched_option.get_attribute("value") or ""
        logging.info(f"🎯 Matched option: '{option_text}' value='{option_value}'")
        ...
        try:
            ...
            selected_text = _normalize_dropdown_text(selected.text)
            selected_value = _normalize_dropdown_text(selected.get_attribute("value"))
            target_normalized = _normalize_dropdown_text(target_text)
            
            if (selected_text == target_normalized or selected_value == target_normalized or target_normalized in selected_text):
                logging.info(f"✅ Dropdown selected: {target_text}")
                return True
        except Exception as e:
            logging.debug(f"Selection verification warning: {e}")
            
        logging.info(f"✅ Dropdown selection executed: {target_text}")
        return True
    except (StaleElementReferenceException, NoSuchElementException) as e:
        logging.warning(f"Dropdown stale/no element: {target_text} | {e}")
    except Exception as e:
        logging.warning(f"Dropdown selection error: {target_text} | {e}")
    return False
...
def fill_initial_rtps_form(driver, user_data, service_type, photo_path, chat_id=None, user_id=None):
    ...
    if user_data.get("salutation"):
        try:
            select_dropdown_robustly(
                driver, By.XPATH, 
                "//select[contains(translate(@id,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'salutation') or contains(translate(@name,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'salutation')]", 
                user_data.get("salutation"), timeout=8, chat_id=chat_id
            )
        except Exception as e:
    ...
    # ============================================================
    # ADDRESS & LOCATION FIELDS (Updated Block with Label Fallback)
    # ============================================================
    logging.info("🏠 Starting Address & Location fields...")
    try:
        state_xpath = (
            "//select["
            "contains(translate(@id,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'state') "
            "or "
            "contains(translate(@name,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'state')"
            "]"
            " | "
            "//label[contains("
        ...
        if not select_dropdown_robustly(driver, By.XPATH, state_xpath, "बिहार", timeout=20, chat_id=chat_id):
            logging.warning("⚠️ Bihar state select नहीं हुआ")
        time.sleep(1)
        ...
        if district:
            district_xpath = (
                "//select["
                "contains(translate(@id,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'district') "
                "or "
                "contains(translate(@name,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'district')"
                "]"
                " | "
                "//label[contains("
                "translate(normalize-space(.),"
            ...
            if select_dropdown_robustly(driver, By.XPATH, district_xpath, district, timeout=25, chat_id=chat_id):
                logging.info(f"✅ District selected: {district}")
            else:
                logging.warning(f"⚠️ District select नहीं हुआ: {district}")
        time.sleep(1.5)
        ...
        if sub_division:
            sub_xpath = (
                "//select["
                "contains(translate(@id, 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'sub') "
                "or "
                "contains(translate(@id, 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'subdivision') "
                "or "
                "contains(translate(@name, 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'sub') "
                "or "
                "contains(translate(@name, 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'subdivision')"
                "]"
            )
            if select_dropdown_robustly(driver, By.XPATH, sub_xpath, sub_division, timeout=25, chat_id=chat_id):
                logging.info(f"✅ Sub-Division selected: {sub_division}")
            else:
                logging.warning(f"⚠️ Sub-Division select नहीं हुआ: {sub_division}")
        time.sleep(1.5)
        ...
        if block:
            block_xpath = (
                "//select["
                "contains(translate(@id, 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'block') "
                "or "
                "contains(translate(@id, 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'circle') "
                "or "
                "contains(translate(@name, 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'block') "
                "or "
                "contains(translate(@name, 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'circle')"
                "]"
            )
            if select_dropdown_robustly(driver, By.XPATH, block_xpath, block, timeout=25, chat_id=chat_id):
                logging.info(f"✅ Block selected: {block}")
            else:
                logging.warning(f"⚠️ Block select नहीं हुआ: {block}")
        time.sleep(1.5)

        local_body_type = user_data.get("local_body_type")
        if local_body_type:
            local_body_xpath = (
                "//select["
                "contains(translate(@id, 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'localbody') "
                "or "
                "contains(translate(@id, 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'bodytype') "
                "or "
                "contains(translate(@name, 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'localbody') "
                "or "
                "contains(translate(@name, 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'bodytype')"
                "]"
            )
            if select_dropdown_robustly(driver, By.XPATH, local_body_xpath, local_body_type, timeout=20, chat_id=chat_id):
                logging.info(f"✅ Local Body Type selected: {local_body_type}")
            else:
                logging.warning(f"⚠️ Local Body Type select नहीं हुआ: {local_body_type}")
        time.sleep(1.5)

        panchayat = user_data.get("panchayat")
        if panchayat:
            panchayat_xpath = (
                "//select["
                "contains(translate(@id, 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'panchayat') "
                "or "
                "contains(translate(@id, 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'gram') "
                "or "
                "contains(translate(@name, 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'panchayat') "
                "or "
                "contains(translate(@name, 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'gram')"
                "]"
            )
            if select_dropdown_robustly(driver, By.XPATH, panchayat_xpath, panchayat, timeout=25, chat_id=chat_id):
                logging.info(f"✅ Panchayat selected: {panchayat}")
            else:
                logging.warning(f"⚠️ Panchayat select नहीं हुआ: {panchayat}")
        time.sleep(1)

        village = user_data.get("village")
        if village:
            find_and_interact(driver, ["//input[contains(translate(@id,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'village') or contains(translate(@name,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'village')]"], "type", village, chat_id=chat_id)

        ward_no = user_data.get("ward_no")
        if ward_no:
            find_and_interact(driver, ["//input[contains(translate(@id,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'ward') or contains(translate(@name,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'ward')]"], "type", ward_no, chat_id=chat_id)

        post_office = user_data.get("post_office")
        if post_office:
            find_and_interact(driver, ["//input[contains(translate(@id,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'postoffice') or contains(translate(@name,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'postoffice')]"], "type", post_office, chat_id=chat_id)

        police_station = user_data.get("police_station")
        if police_station:
            find_and_interact(driver, ["//input[contains(translate(@id,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'policestation') or contains(translate(@name,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'policestation')]"], "type", police_station, chat_id=chat_id)

        pin_code = user_data.get("pin_code")
        if pin_code:
            find_and_interact(driver, ["//input[contains(translate(@id,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'pincode') or contains(translate(@name,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'pincode')]"], "type", pin_code, chat_id=chat_id)
            
        logging.info("✅ Address & Location fields processing completed.")
    except Exception as e:
        logging.warning(f"⚠️ Address fields filling warning: {e}")
        try:
            debug_screenshot(driver, chat_id, "ADDRESS_FIELDS_ERROR")
        except Exception:
            pass
    ...
    # Service-specific extra fields (Profession, Purpose, Category, Caste, Annual Income, Residence Type)
    try:
        if user_data.get("profession"):
            select_dropdown_robustly(driver, By.XPATH, "//select[contains(translate(@id,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'profession')]", user_data.get("profession"), timeout=8, chat_id=chat_id)
        if user_data.get("category"):
            select_dropdown_robustly(driver, By.XPATH, "//select[contains(translate(@id,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'category')]", user_data.get("category"), timeout=8, chat_id=chat_id)
        if user_data.get("caste"):
            select_dropdown_robustly(driver, By.XPATH, "//select[contains(translate(@id,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'caste')]", user_data.get("caste"), timeout=8, chat_id=chat_id)
        if user_data.get("annual_income"):
            find_and_interact(driver, ["//input[contains(translate(@id,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'income') or contains(translate(@name,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'income')]"], "type", user_data.get("annual_income"), chat_id=chat_id)
        if user_data.get("residence_type"):
            select_dropdown_robustly(driver, By.XPATH, "//select[contains(translate(@id,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'residencetype')]", user_data.get("residence_type"), timeout=8, chat_id=chat_id)
    except Exception as e:
        logging.warning(f"Service specific fields warning: {e}")

    # Aadhaar Number filling
    if user_data.get("aadhaar_number"):
        try:
            find_and_interact(driver, [
                "//input[@id='aadhaar_no']",
                "//input[contains(translate(@id,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'aadhaar')]",
                "//input[contains(translate(@name,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'aadhaar')]"
            ], "type", user_data["aadhaar_number"], chat_id=chat_id)
            logging.info("✅ Aadhaar Number filled successfully.")
        except Exception as e:
            logging.warning(f"Aadhaar number filling warning: {e}")
    ...
    if user_data.get("email"):
        try:
            ...
            email_xpaths = [
                ...
                "//*[contains(translate(normalize-space(.),'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'email')]/following::input[1]"
            ]
            ...
            find_and_interact(
                ...
                email_xpaths,
                action_type="type",
                text_value=email_value,
                timeout=15,
                chat_id=chat_id
            )
            logging.info("✅ Email address filled successfully.")
            debug_screenshot(driver, chat_id, "AFTER_EMAIL")
        except Exception as e:
    ...
    try:
        ...
        # --------------------------------------------------------
        # IMPORTANT: ENTER TRIGGER
        # --------------------------------------------------------
        try:
            mobile_element.send_keys(Keys.ENTER)
            logging.info("📱 ENTER pressed after mobile number.")
        except Exception as enter_error:
            logging.warning(f"Normal ENTER failed: {enter_error}")
            try:
                ...
                logging.info("📱 JavaScript ENTER event dispatched.")
            except Exception as js_error:
                logging.warning(f"JS ENTER trigger failed: {js_error}")

        debug_screenshot(driver, chat_id, "AFTER_MOBILE_ENTER")
        time.sleep(2)
        ...
        if detect_mobile_otp_popup(driver, timeout=10):
            ...
            bot.send_message(
                chat_id,
                "📱 मोबाइल नंबर पर OTP भेजा गया है। कृपया OTP दर्ज करें:"
            )
            if user_id:
                bot.set_state(user_id, RTPSState.mobile_otp_input, chat_id)
            return "MOBILE_OTP"
        else:
            logging.warning("⚠️ ENTER के बाद Mobile OTP popup नहीं मिला।")

    except Exception as e:
        logging.warning(f"⚠️ Mobile number / ENTER trigger warning: {e}")

# ===================================================================
# Safe Step Wrapper & Sequential Form Filling Flow
# ===================================================================

def safe_step(bot, chat_id, step_name, func, is_mandatory=False):
    try:
        logging.info("➡️ STEP START: %s", step_name)
        func()
        logging.info("✅ STEP DONE: %s", step_name)
        return True
    except Exception as e:
        logging.warning("⚠️ STEP FAILED/SKIPPED: %s | %s: %s", step_name, type(e).__name__, str(e))
        
        markup = telebot.types.InlineKeyboardMarkup()
        markup.add(
            telebot.types.InlineKeyboardButton("➡️ Next Step", callback_data="rtps_next_step")
        )
        
        mandatory_warning = (
            "\n🔴 **ध्यान दें:** यह field अनिवार्य (Mandatory) है। कृपया इसे पोर्टल पर manually भरें!"
            if is_mandatory 
            else "\n⚪ यह optional लग रहा है, चाहें तो छोड़कर आगे बढ़ सकते हैं।"
        )
        
        bot.send_message(
            chat_id,
            f"⚠️ **{step_name}** automatic नहीं हुआ।{mandatory_warning}\n"
            f"यदि मैन्युअली भरकर आगे बढ़ना है, तो नीचे दिए गए बटन को दबाएँ:",
            reply_markup=markup,
            parse_mode="Markdown"
        )
        return False
...
def process_service_type(message):
    ...
    bot.send_message(message.chat.id, f"सर्विस '{service}' चुनी गई। विवरण नीचे दिए गए JSON प्रारूप में भरें (Aadhaar number सहित):\n\n`{templates[service]}`", parse_mode="Markdown")      
...
def process_mobile_otp_input(message):
    ...
    try:      
        ...
        logging.info(f"Searching Mobile OTP input with {len(otp_xpaths)} XPath(s)")      
        ...
        debug_screenshot(driver, chat_id, "OTP_ENTERED_PAGE")
          
        validate_btn_xpaths = [      
            "//button[@id='btnValidateOtp']",
            "//input[@id='btnValidateOtp']",
            "//div[contains(@class,'modal') or contains(@role,'dialog')]//button[contains(normalize-space(.),'Validate')]",      
            "//div[contains(@class,'modal') or contains(@role,'dialog')]//input[@type='button' or @type='submit' and contains(@value,'Validate')]",      
            ...
            "//a[contains(normalize-space(.),'Validate')]"      
        ]      
        
        logging.info("Searching Mobile OTP Validate button")
        time.sleep(1)
        ...
        try:
            ...
            logging.info("OTP alert: %s", mask_sensitive_data(alert_text))
            
            if "verified" in alert_text.lower():
                otp_verified_successfully = True
                logging.info("✅ Alert contains 'Verified'. OTP SUCCESS.")
            else:
                logging.warning("⚠️ Alert did not contain 'Verified': %s", mask_sensitive_data(alert_text))
            
            alert.accept()
        except TimeoutException:
            logging.info("No OTP alert appeared, checking DOM or Please wait state")
        except AlertNotPresentException:
            pass

        try:
            send_step_screenshot(driver, chat_id, "PLEASE_WAIT_SCREEN_CAPTURED")
        except Exception:
            pass
        ...
        try:
            send_step_screenshot(driver, chat_id, "MOBILE_OTP_SUCCESS")
        except Exception:
            pass

        time.sleep(2)      
        debug_screenshot(driver, chat_id, "AFTER_VALIDATE_CLICK")
        ...
        logging.info("Mobile OTP verification result: %s", otp_result)  
        ...
        if otp_result == "INVALID":  
            ...
            if attempts >= MAX_OTP_ATTEMPTS:  
                bot.send_message(  
                    chat_id,   
                    f"❌ अधिकतम मोबाइल OTP प्रयास ({MAX_OTP_ATTEMPTS}) समाप्त हुए।"  
                )  
                finish_and_cleanup_session(chat_id, session)  
                return  
            bot.send_message(  
                chat_id,   
                f"❌ OTP गलत है या सत्यापित नहीं हुआ। पुनः OTP दर्ज करें (प्रयास {attempts}/{MAX_OTP_ATTEMPTS}):"  
            )  
            return  
        ...
        if otp_result != "SUCCESS":  
            ...
            bot.send_message(  
                chat_id,   
                "⚠️ OTP verification का स्पष्ट परिणाम नहीं मिला। ऊपर दिए गए स्क्रीनशॉट से जाँच करें।"  
            )  
            send_error_screenshot(chat_id, driver, "⚠️ OTP Verification Result Unknown")  
            return  

        session["is_processing"] = False  
        session["last_activity"] = time.time()  
        bot.send_message(  
            chat_id, "✅ Mobile OTP सत्यापित हो गया।"  
        )  
        ...
        debug_screenshot(driver, chat_id, "NEW_PAGE_AFTER_OTP_CLOSED")
    ...
    try:      
        ...
        try:      
            attach_xpaths = [
                ...
                "//button[contains(normalize-space(.), 'Attach')]",
