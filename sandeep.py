...
# -------------------------------------------------------------------    
def find_and_interact(driver, xpaths, action_type="click", text_value=None, timeout=15):    
    last_err = None    
    for xp in xpaths:    
        try:    
            ...
            if action_type == "click":    
                try:
                    elem = WebDriverWait(driver, 5).until(EC.element_to_be_clickable((By.XPATH, xp)))
                    elem.click()
                except Exception:
    ...
    # Check iFrames if element not found in main DOM
    if action_type != "switch_frame":
        if check_and_switch_iframe(driver, xpaths):
            return find_and_interact(driver, xpaths, action_type, text_value, timeout=5)
            ...
            try:
                ...
                for xp in xpaths:
                    if len(driver.find_elements(By.XPATH, xp)) > 0:
                        logging.info(f"✅ Found target inside iFrame index [{index}]")
                        return True
            except Exception:
                continue
    except Exception:
