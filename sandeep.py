# ===================================================================
# Python Telegram Bot Source Code (Updated with Robust Dropdown Logic)
# ===================================================================
...
def wait_for_dropdown_target(driver, locator, target_text, timeout=30):
    """
    जब तक dependent dropdown में target option load नहीं होता, तब तक wait करता है।
    """
    if not target_text:
        return None
    ...
    while time.time() < end_time:
        try:
            ...
            logging.info(f"⏳ Dropdown में '{target_text}' अभी load नहीं हुआ...")
        except (NoSuchElementException, StaleElementReferenceException):
            pass
        except Exception as e:
            logging.debug(f"Dropdown target wait: {repr(e)}")
        time.sleep(1)
        
    raise TimeoutException(f"Dropdown में target '{target_text}' {timeout}s में load नहीं हुआ")
...
def continue_rtps_form_after_mobile_otp(
    ...
    try:
        ...
        if chat_id:
            bot.send_message(chat_id, "🔄 फॉर्म डिटेल्स भरना शुरू हो रहा है (Dependent Dropdown Sequence)...")

        # XPaths for Dropdowns
        state_xpath = "//select[@id='state' or contains(@name, 'state')]"
        dist_xpath = "//select[@id='district' or contains(translate(@name,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'district')]"
        sub_xpath = "//select[@id='sub_division' or contains(translate(@name,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'sub')]"
        block_xpath = "//select[@id='block' or contains(translate(@name,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'block') or contains(translate(@name,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'circle')]"

        # 1. EMAIL
        safe_step(bot, chat_id, "EMAIL", lambda: (
            lambda f=(wait.until(EC.presence_of_element_located((By.XPATH, "//input[contains(@name,'email') or contains(@id,'email')]")))): 
            (f.clear(), f.send_keys(user_data.get("email", "")))
        )(), is_mandatory=False)

        # 2. STATE (Wait + Select)
        safe_step(bot, chat_id, "STATE", lambda: select_dropdown_robustly(
            driver, By.XPATH, state_xpath, "बिहार", timeout=30, min_options=2, chat_id=chat_id
        ), is_mandatory=True)

        # 3. DISTRICT (Wait for Target + Select)
        safe_step(bot, chat_id, "DISTRICT", lambda: (
            wait_for_dropdown_target(driver, dist_xpath, user_data.get("district"), timeout=30),
            select_dropdown_robustly(driver, By.XPATH, dist_xpath, user_data.get("district"), timeout=30, min_options=2, chat_id=chat_id)
        ), is_mandatory=True)

        # 4. SUB-DIVISION
        safe_step(bot, chat_id, "SUB-DIVISION", lambda: (
            wait_for_dropdown_target(driver, sub_xpath, user_data.get("sub_division"), timeout=30),
            select_dropdown_robustly(driver, By.XPATH, sub_xpath, user_data.get("sub_division"), timeout=30, min_options=2, chat_id=chat_id)
        ), is_mandatory=True)

        # 5. BLOCK
        safe_step(bot, chat_id, "BLOCK", lambda: (
            wait_for_dropdown_target(driver, block_xpath, user_data.get("block"), timeout=30),
            select_dropdown_robustly(driver, By.XPATH, block_xpath, user_data.get("block"), timeout=30, min_options=2, chat_id=chat_id)
        ), is_mandatory=True)

        if chat_id:
            send_step_screenshot(driver, chat_id, "AFTER_DROPDOWNS")
        
        logging.info("✅ Dependent dropdowns successfully processed")
