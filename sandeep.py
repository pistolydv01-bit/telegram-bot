...
def process_captcha_input(message):
    ...
    try:
        ...
        # SAME browser tab/window. Do not wait for a new tab as the primary
        # transition signal. Keep the current handle and wait for the document
        # to become ready, then detect the actual Annexure DOM controls.
        try:
        ...
        # First wait for the current document to finish the RTPS reload.
        wait_for_rtps_page_ready(driver, timeout=45)
        ...
        if annexure_detected:
            ...
            # Give the generic page-ready detector one final chance, but never
            # label an unchanged CAPTCHA as wrong merely because URL stayed same.
            wait_for_rtps_page_ready(driver, timeout=15)

        time.sleep(1.0)
        session["last_activity"] = time.time()

        # This is the screenshot the user wants to see after the alert/OK and
        # after the actual next page/tab has loaded.
        send_step_screenshot(
            driver,
            chat_id,
        ...
        # The screenshot above is deliberately taken after the real page/tab
        # transition. The Annexure page has a very distinctive RTPS control:
        #   <button id="submit_btn">Attach Annexure</button>
        # Detect it immediately so the flow never falls into UNKNOWN merely
        # because the generic verification detector missed the new page.
        # Strong same-window Annexure detection. RTPS commonly reloads the
        # current tab without changing the URL, so DOM controls are authoritative.
        ...
        if step == "UNKNOWN":
            ...
            if step == "UNKNOWN":
                ...
                if late_alert_handled:
                    session["last_activity"] = time.time()
                    send_step_screenshot(driver, chat_id, "AFTER_LATE_RTPS_ALERT_OK")
                    switch_to_new_rtps_window_and_wait(
                        driver,
