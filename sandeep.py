...
def process_captcha_input(message):
    ...
    try:
        ...
        # RTPS may open a new browser tab/window only after OK. Switch to it
        # without closing the original window. If no new window exists, remain
        # on the current page and wait for same-window navigation.
        switched_new_window, new_handle, post_alert_url = (
        ...
        if switched_new_window:
            logging.info("✅ New RTPS tab/window selected: %s", new_handle)
        elif post_alert_url and post_alert_url != pre_proceed_url:
            logging.info("✅ RTPS next page loaded in the same window: %s", post_alert_url)
        else:
            # Restore the original handle only when no new tab was created and
            # Selenium is still on a valid window. This prevents accidental
            # switching into an unrelated browser tab.
            try:
        ...
        if step == "UNKNOWN":
            ...
            if step == "UNKNOWN":
                ...
                if step == "UNKNOWN":
                    # A changed window count or URL is itself a real transition.
                    # Route onward instead of resetting the CAPTCHA state.
                    try:
                    ...
                    if current_handles != pre_proceed_handles or (
                        pre_proceed_url and current_url and current_url != pre_proceed_url
                    ):
                        wait_for_rtps_page_ready(driver, timeout=15)
