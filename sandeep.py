...
def process_captcha_input(message):
    ...
    try:
        ...
        # ------------------------------------------------------------
        # Keep the actual CAPTCHA WebElement. On a real same-tab reload this
        # element becomes stale; that is a much stronger transition signal than
        # URL/timeOrigin/body-length checks.
        captcha_input_elem = fill_visible_captcha_input(
            driver,
            captcha_text,
            timeout=20
        )
        ...
        # IMPORTANT: submit_btn is reused by RTPS on CAPTCHA/Annexure/Save pages.
        # Select the visible Proceed control that belongs to the SAME form as the
        # currently visible CAPTCHA input. This keeps V2's original flow intact
        # while preventing a duplicate/hidden submit_btn from being clicked.
        process_btn_xpaths = [
            ...
            "//button[@id='submit_btn' and contains(translate(normalize-space(.),'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'proceed')]",
            "//button[contains(normalize-space(.),'Process')]",
            "//input[@value='Process' or contains(@id, 'btnProcess')]"
        ]

        proceed_target = None
        try:
            driver.switch_to.default_content()
            # Prefer a visible CAPTCHA input and its closest form.
            captcha_form = driver.execute_script("""
                const c = arguments[0];
                const f = c ? c.closest('form') : null;
                return f || null;
            """, captcha_input_elem)
            candidates = []
            for xp in process_btn_xpaths:
                try:
                    candidates.extend(driver.find_elements(By.XPATH, xp))
                except Exception:
                    pass
            candidates = [e for e in candidates if e.is_displayed() and e.is_enabled()]
            # De-duplicate WebElements by DOM identity.
            unique = []
            seen = set()
            for e in candidates:
                try:
                    key = driver.execute_script("return arguments[0];", e)
                    # WebElement itself is hashable in Selenium; fallback to id.
                    marker = getattr(e, 'id', None) or str(id(e))
            ...
            # First choice: Proceed whose closest form is the CAPTCHA form.
            if captcha_form is not None:
                for e in candidates:
                    try:
                        same_form = driver.execute_script(
                            "return arguments[0].closest('form') === arguments[1];",
                            e, captcha_form
                        )
                        if same_form:
                            proceed_target = e
                        ...
                        continue
            if proceed_target is None and candidates:
                proceed_target = candidates[0]

            if proceed_target is not None:
                logging.info(
                    "🎯 RTPS CAPTCHA Proceed target: tag=%s id=%s type=%r text=%r onclick=%r disabled=%r same_form=%s",
                    proceed_target.tag_name, proceed_target.get_attribute('id'),
                    proceed_target.get_attribute('type'), proceed_target.text,
                    proceed_target.get_attribute('onclick'), proceed_target.get_attribute('disabled'),
                    bool(captcha_form and driver.execute_script(
                        "return arguments[0].closest('form') === arguments[1];",
                        proceed_target, captcha_form
                    ))
                )
        except Exception as e:
            logging.debug("Proceed target probe failed: %r", e)

        if proceed_target is not None:
            try:
                driver.execute_script("arguments[0].scrollIntoView({block:'center'});", proceed_target)
                time.sleep(0.3)
                proceed_target.click()
            except Exception:
                # Normal browser-equivalent Selenium click fallback only; do not
                # submit the form directly because RTPS client-side validation must run.
                from selenium.webdriver.common.action_chains import ActionChains
                try:
                    ActionChains(driver).move_to_element(proceed_target).click().perform()
                except Exception:
                    driver.execute_script("arguments[0].click();", proceed_target)
        else:
            find_and_interact(
                driver,
                process_btn_xpaths,
        ...
        # ------------------------------------------------------------
        # A real browser alert should appear shortly after Proceed. Do not block
        # the Telegram session for minutes when there is no alert at all.
        ...
        # If the JavaScript alert itself says CAPTCHA is invalid, the current
        # CAPTCHA page is the expected result. Stop immediately instead of
        # waiting 60+ seconds and falsely labelling that same screenshot as
        # NEXT_PAGE.
        if alert_handled and alert_kind == "CAPTCHA_ERROR":
            ...
            send_step_screenshot(
                driver,
                chat_id,
                "FINAL_CAPTCHA_REJECTED_FULL_SCREEN" if is_final_captcha
                else "CAPTCHA_REJECTED_FULL_SCREEN"
            )

            bot.send_message(
                chat_id,
                "❌ RTPS ने CAPTCHA को गलत बताया है।\n"
            ...
            current_captcha_path = os.path.join(
                user_dir,
                f"captcha_rejected_{int(time.time()*1000)}.png"
            )
            try:
                if capture_captcha_image(
                    ...
                    with open(current_captcha_path, "rb") as c_img:
                        bot.send_photo(
                            chat_id,
                            c_img,
                            caption="🧩 Page पर नया CAPTCHA है। इसे दर्ज करें।",
                            reply_markup=captcha_inline_keyboard()
                        )
            finally:
                try:
        ...
        # RTPS transfers to the next page by reloading/navigating in the
        # SAME browser tab/window. Do not treat the already-complete old
        # document as the next page. First wait for the actual reload/DOM
        # replacement triggered by Proceed + alert OK.
        try:
            if pre_proceed_handle and pre_proceed_handle in driver.window_handles:
                driver.switch_to.window(pre_proceed_handle)
        except Exception:
            pass

        # PRIMARY same-tab reload barrier: the old CAPTCHA input must become
        # stale/disappear. If it remains, RTPS has NOT replaced the document.
        # This also cleanly distinguishes a rejected CAPTCHA from a real reload.
        same_window_changed = False
        try:
            WebDriverWait(driver, 30, poll_frequency=0.25).until(
                EC.staleness_of(captcha_input_elem)
            )
            same_window_changed = True
            logging.info("🔄 RTPS same-tab reload confirmed: old CAPTCHA input became stale.")
        except TimeoutException:
            # Some RTPS responses replace the form without making Selenium's
            # original node stale. Use the old signature only as a secondary
            # fallback, never as proof of Annexure by itself.
            logging.warning("⚠️ Old CAPTCHA input did not become stale within 30s; checking secondary transition signal.")
        ...
        logging.info("🔄 Same-window RTPS transition result: %s", same_window_changed)

        # RTPS can keep the same URL while replacing the page contents, so URL
        # change alone is NOT treated as the next-page signal.
        annexure_xpaths = [
            "//button[@id='submit_btn' and contains(normalize-space(.),'Attach Annexure')]",
            ...
            "//button[@id='submit_btn' and contains(normalize-space(.),'Save Annexure')]",
            "//*[contains(normalize-space(.),'Save Annexure')]",
        ]
        ...
        def _annexure_controls_present():
            ...
            # Also inspect iframes because RTPS may render the transferred
            # page inside a frame after the reload.
            try:
        ...
        if annexure_detected:
            logging.info("✅ Actual RTPS Annexure page detected after same-window reload.")

            time.sleep(1.0)
            session["last_activity"] = time.time()

            # IMPORTANT: only send NEXT_PAGE screenshot after the exact Annexure
            # controls are actually present. The previous version sent this
            # screenshot even when the old CAPTCHA page was still on screen.
            send_step_screenshot(
                driver,
                chat_id,
            ...
            # Give the generic detector a final short chance. Do NOT call the
            # unchanged CAPTCHA page a next page.
            wait_for_rtps_page_ready(driver, timeout=8)
            immediate_step = detect_post_proceed_state(
                driver, previous_url=pre_proceed_url, timeout=8
            )
            if immediate_step == "ANNEXURE":
                annexure_detected = True
                time.sleep(0.8)
                send_step_screenshot(
                    driver,
        ...
        # Strong same-window Annexure detection. RTPS commonly reloads the
        # current tab without changing the URL, so the exact DOM controls are
        # authoritative.
        immediate_step = "ANNEXURE" if annexure_detected else detect_post_proceed_state(
            driver, previous_url=pre_proceed_url, timeout=5
        )
        if immediate_step == "ANNEXURE":
            session["is_processing"] = False
            session["last_activity"] = time.time()
            bot.send_message(
                chat_id,
                "📎 Actual Annexure page detect हो गया।\n"
                "➡️ अब Attach Annexure और documents वाला step शुरू हो रहा है..."
            )
            execute_final_submission_internal(chat_id, session, "")
            return

        # Some RTPS versions show a Mobile/Email verification HTML modal
        # with an OK button after the page transition. Handle that before
        # checking the verification state.
        click_post_proceed_ok_if_present(driver, chat_id=chat_id, timeout=15)
        time.sleep(1.5)
        ...
        if step == "UNKNOWN":
            # V2-preserved bounded fallback: if no alert was actually present,
            # give same-tab DOM replacement a short chance, then report the real
            # state. Never claim that an alert was accepted when none existed.
            if not alert_handled:
                ...
                try:
                    ...
                    same_window_changed = True
                    logging.info("🔄 RTPS same-tab reload confirmed after Proceed: old CAPTCHA input became stale.")
                except StaleElementReferenceException:
                    same_window_changed = True
                except TimeoutException:
                    same_window_changed = False
                except Exception as e:
                    logging.debug("Short same-tab reload wait: %r", e)

                if not same_window_changed:
                    try:
                        same_window_changed = wait_for_rtps_same_window_transition(
                            driver, before_signature=pre_proceed_signature, timeout=8
                        )
                    except Exception:
                        same_window_changed = False
                ...
                if step == "UNKNOWN":
                    session["is_processing"] = False
                    session["last_activity"] = time.time()
                    send_step_screenshot(driver, chat_id, "CAPTCHA_PROCEED_NO_ALERT_OR_STATE_CHANGE")
                    bot.send_message(
                        chat_id,
                        "⚠️ Proceed click हुआ, लेकिन RTPS ने कोई JavaScript alert नहीं दिखाया "
                        "और same-tab next state भी अभी नहीं मिला।\n"
                        "📸 Diagnostic screenshot भेजा गया है।\n"
                        "❗ इसे CAPTCHA गलत नहीं माना गया है।"
                    )
                    bot.set_state(user_id, RTPSState.captcha_input, chat_id)
                    return

            # If an alert was handled but the page detector was early, retain V2's
            # existing short post-alert state checks.
            if step == "UNKNOWN":
                time.sleep(1)
                step = detect_post_proceed_state(
                    driver, previous_url=pre_proceed_url, timeout=12
                )
            ...
            if step == "UNKNOWN":
                ...
                bot.send_message(
                    chat_id,
                    "⚠️ RTPS ने अभी next state नहीं दिखाया।\n"
                    "📸 Current diagnostic screenshot भेजा गया है।\n"
...
# ===================================================================
# WhatsApp Cloud API Webhook + Render Health Endpoint
# ===================================================================
...
#
# Flask owns Render's PORT. Telegram polling runs in a background thread.
# ===================================================================

app = Flask(__name__)

VERIFY_TOKEN = os.getenv("VERIFY_TOKEN", "my_secret_token_123").strip()
...
def health_check():
    """Render health check / basic service check."""
    return "OK", 200
...
def verify_webhook():
    ...
    if mode == "subscribe" and token == VERIFY_TOKEN and challenge:
        logging.info("WhatsApp webhook verification successful")
...
def send_whatsapp_message(phone_num_id, to, text):
    """Send a text message through the WhatsApp Cloud API."""
    if not WHATSAPP_TOKEN:
        logging.error("WHATSAPP_TOKEN is not configured")
        return False

    url = (
        f"https://graph.facebook.com/"
        f"{WHATSAPP_GRAPH_VERSION}/{phone_num_id}/messages"
    )
    ...
    try:
        ...
        if 200 <= response.status_code < 300:
            logging.info("WhatsApp reply sent successfully")
            return True

        logging.error(
            "WhatsApp API error: status=%s body=%s",
            response.status_code,
            mask_sensitive_data(response.text[:1000]),
        )
        return False

    except requests.RequestException as e:
        logging.error("WhatsApp API request failed: %s", e)
        return False
...
def process_whatsapp_text_message(
    ...
    """
    Central WhatsApp message handler.

    फिलहाल incoming text का safe echo reply भेजता है।
    बाद में यहाँ existing RTPS/business logic जोड़ा जा सकता है।
    """
    ...
    reply_text = f"Bot Reply: Aapne bheja '{user_text}'"
...
def handle_whatsapp_messages():
    """Receive incoming WhatsApp Cloud API webhook events."""
    try:
        ...
        for entry in entries:
            for change in entry.get("changes") or []:
                ...
                # Delivery/read/status events normally have no messages.
                if not messages:
                    continue
                ...
                if not phone_num_id:
                    logging.warning(
                        "WhatsApp event में phone_number_id नहीं मिला"
                    )
                    continue
                ...
                for message in messages:
                    ...
                    if message_type == "text":
                        ...
                        logging.info(
                            "WhatsApp message type '%s' अभी handled नहीं है",
                            message_type,
                        )
        ...
        # Avoid unnecessary webhook retries for a received event.
        return "OK", 200


def run_flask_server():
    """Run Flask/Render HTTP server in a background thread."""
    port = int(os.getenv("PORT", "10000"))

    logging.info(
        "Flask/WhatsApp Webhook Server Running on Port %s",
        port,
    )
...
if __name__ == "__main__":
    # Flask must own Render's PORT while Telegram polling continues.
    threading.Thread(
