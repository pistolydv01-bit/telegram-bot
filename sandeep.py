# ===================================================================
# Python Telegram Bot Source Code (Updated with Aadhaar Field Automation)
# ===================================================================
...
def continue_rtps_form_after_mobile_otp(
    ...
    try:
        ...
        # ===================================================================
        # आधार संख्या (Aadhaar Number) भरने का चरण
        # ===================================================================
        if user_data.get("aadhaar_no"):
            safe_step(bot, chat_id, "AADHAAR_NUMBER", lambda: find_and_interact(
                driver, [
                    ...
                    "//label[contains(normalize-space(.),'Aadhaar Number')]/following::input[1]",
                    "//label[contains(normalize-space(.),'आधार संख्या')]/following::input[1]"
                ],
                action_type="type",
                text_value=user_data.get("aadhaar_no"),
                timeout=15,
                chat_id=chat_id
            ), is_mandatory=False)
        ...
        logging.info("✅ Dependent dropdowns & Aadhaar successfully processed")
    ...
    try:      
        ...
        if otp_result == "INVALID":  
            ...
            if attempts >= MAX_OTP_ATTEMPTS:  
                ...
                finish_and_cleanup_session(chat_id, session)  
                return  
            bot.send_message(chat_id, f"❌ OTP गलत है या सत्यापित नहीं हुआ। पुनः OTP दर्ज करें (प्रयास {attempts}/{MAX_OTP_ATTEMPTS}):")  
            return  

        if otp_result != "SUCCESS":  
            session["is_processing"] = False  
            session["last_activity"] = time.time()  
            bot.send_message(chat_id, "⚠️ OTP verification का स्पष्ट परिणाम नहीं मिला। ऊपर दिए गए स्क्रीनशॉट से जाँच करें।")  
            send_error_screenshot(chat_id, driver, "⚠️ OTP Verification Result Unknown")  
            return  

        session["is_processing"] = False  
        session["last_activity"] = time.time()  
        bot.send_message(chat_id, "✅ Mobile OTP सत्यापित हो गया। फॉर्म आगे भरा जा रहा है...")  
        ...
        bot.send_message(chat_id, f"❌ Mobile OTP के बाद error आया: {str(e)[:500]}")
