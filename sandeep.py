...
def wait_for_rtps_same_window_transition(driver, before_signature=None, timeout=60):
    """Wait for the REAL RTPS same-window transition.

    RTPS normally reloads/replaces the CURRENT tab after Proceed + alert OK;
    it does not have to create a new tab and it may keep the same URL.
    Therefore readyState/URL alone are never treated as success.
    """
    before_signature = before_signature or {}
    deadline = time.time() + timeout
    saw_navigation_change = False
    ...
    while time.time() < deadline:
        try:
            ...
            if (before_signature.get("timeOrigin") and
                    current.get("timeOrigin") and
                    current.get("timeOrigin") != before_signature.get("timeOrigin")):
                saw_navigation_change = True

            if (before_signature.get("bodyLength") is not None and
                    current.get("bodyLength") is not None and
                    abs(current.get("bodyLength", 0) - before_signature.get("bodyLength", 0)) > 80):
                saw_dom_change = True

            if (before_signature.get("bodyHead") and current.get("bodyHead") and
                    current.get("bodyHead") != before_signature.get("bodyHead")):
                saw_dom_change = True

            if saw_navigation_change or saw_dom_change:
                logging.info(
                    "RTPS transition observed: navigation=%s dom=%s",
                    saw_navigation_change, saw_dom_change
                )
                break
        except Exception as e:
    ...
    # If a real navigation/reload was observed, wait for the NEW document.
    if saw_navigation_change or saw_dom_change:
        wait_for_rtps_page_ready(driver, timeout=25)
        time.sleep(1.0)
    else:
        # Do not falsely declare the old CAPTCHA page as the next page.
        logging.warning("No generic RTPS navigation signature detected; concrete DOM detection will decide state.")
        time.sleep(1.0)

    return saw_navigation_change or saw_dom_change


def _rtps_annexure_dom_present(driver):
    """Authoritative detector for the actual Annexure page supplied by the user."""
    selectors = [
        (By.ID, "7071_enclDoc_cb"),
        (By.ID, "3354_enclDoc_cb"),
        (By.ID, "7071_attach"),
        (By.ID, "3354_attach"),
    ]

    # The supplied RTPS HTML contains BOTH document rows. Requiring the two
    # select controls makes this substantially safer than looking for a generic
    # word such as 'Save' or relying on URL changes.
    try:
        driver.switch_to.default_content()
        if all(any(e.is_displayed() for e in driver.find_elements(by, value))
               for by, value in selectors[:2]):
            return True
    except Exception:
        pass

    # Some RTPS builds render the form inside an iframe.
    try:
...
def wait_for_actual_rtps_next_page(driver, before_signature=None, before_handles=None, timeout=75):
    """Wait for RTPS's actual post-CAPTCHA page, including same-window reload.

    Order is deliberate:
      1. detect a newly-created window and switch to it if one really appears;
      2. wait until that document is complete;
      3. otherwise wait for the current-tab reload/DOM replacement;
      4. finally require the concrete Annexure DOM from the supplied HTML.

    This function never calls URL_CHANGED, readyState=complete, or a changed
    screenshot by itself a successful next-page state. The Annexure controls are
    the authoritative success marker for this transition.
    """
    before_handles = set(before_handles or [])
    deadline = time.time() + timeout
    ...
    while time.time() < deadline:
        # A) New tab/window, only if RTPS actually created one.
        try:
            ...
            if new_handles:
                ...
                logging.info("RTPS opened a new window/tab; switched to it.")
                wait_for_rtps_page_ready(driver, timeout=25)
        except Exception as e:
            logging.debug("RTPS new-window check: %r", e)

        # B) The actual page supplied by the user is the decisive marker.
        if _rtps_annexure_dom_present(driver):
            logging.info("Actual RTPS Annexure page detected.")
            return "ANNEXURE", switched_to_new

        # C) If no new window exists, the portal is expected to reload this tab.
        try:
            current = get_rtps_page_signature(driver)
            if before_signature and current:
                nav_changed = (
                ...
                head_changed = (
                    before_signature.get("bodyHead") and current.get("bodyHead") and
                    current.get("bodyHead") != before_signature.get("bodyHead")
                )
                if nav_changed or body_changed or head_changed:
                    wait_for_rtps_page_ready(driver, timeout=15)
        except Exception as e:
            logging.debug("RTPS same-window state probe: %r", e)

        time.sleep(0.5)

    # Restore original tab only when it still exists; never invent a switch.
    if not switched_to_new and original_handle:
        try:
    ...
    if _rtps_annexure_dom_present(driver):
        return "ANNEXURE", switched_to_new
    return "UNKNOWN", switched_to_new
...
def process_captcha_input(message):
    ...
    try:
        ...
        # ------------------------------------------------------------
        # 4) Wait for the REAL RTPS next page.
        #    IMPORTANT: this portal normally reloads the SAME tab. A new tab is
        #    handled only if one actually appears. URL/readyState alone are not
        #    enough; the supplied Annexure DOM is the authoritative marker.
        # ------------------------------------------------------------
        next_state, switched_to_new = wait_for_actual_rtps_next_page(
            driver,
            before_signature=pre_proceed_signature,
            before_handles=pre_proceed_handles,
            timeout=75
        )
        logging.info("RTPS actual next-page result: state=%s new_window=%s", next_state, switched_to_new)

        if next_state == "ANNEXURE":
            session["is_processing"] = False
            session["last_activity"] = time.time()
            send_step_screenshot(
                driver,
            ...
            bot.send_message(
                ...
                "📎 Attach Annexure page मिल गया है।\n"
                "➡️ अब Annexure/document वाला step शुरू हो रहा है..."
            )
            execute_final_submission_internal(chat_id, session, "")
            return

        # No concrete next-page DOM appeared. Do NOT send a misleading
        # 'next-page' screenshot and do NOT call this a CAPTCHA failure.
        session["is_processing"] = False
        session["last_activity"] = time.time()
        send_step_screenshot(
            driver,
            chat_id,
            "CAPTCHA_PROCEED_NO_STATE_CHANGE"
        )
        bot.send_message(
            chat_id,
            "⚠️ Proceed और alert-OK पूरा हो गया है, लेकिन RTPS का actual next page detect नहीं हुआ।\n"
            "❗ CAPTCHA को गलत नहीं माना गया है।\n"
            "⏳ Portal ने same-window transition पूरा नहीं किया या Annexure DOM नहीं आया।\n"
            "📸 Screenshot भेज दिया गया है; script पुराने CAPTCHA page को next page नहीं मानेगी।"
        )
        return
