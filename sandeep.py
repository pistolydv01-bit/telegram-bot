...
def select_dropdown_robustly(driver, element_by, locator_value, target_text, timeout=20, min_options=2):    
    ...
    while time.time() < end_time:    
        try:    
            ...
            continue    
        time.sleep(0.5)    
    raise ValueError(f"ड्रॉपडाउन में विकल्प '{target_text}' लोड नहीं हो सका।")    

def find_and_interact(driver, xpaths, action_type="click", text_value=None, timeout=25):    
    wait = WebDriverWait(driver, timeout)    
    last_err = None    
    for xp in xpaths:    
        try:    
...
def fill_initial_rtps_form(driver, user_data, service_type, photo_path):    
    ...
    find_and_interact(driver, [    
        ...
        "//a[contains(@href, 'General')]"
    ])    
    time.sleep(2)

    service_map = {    
        "RESIDENCE": [
            "//a[contains(text(), 'आवासीय प्रमाण पत्र')]", 
            "//a[contains(text(), 'Issuance of Residence Certificate')]",
            "//a[contains(translate(text(), 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'residence')]"
        ],    
        "CASTE": [
            "//a[contains(text(), 'जाति प्रमाण पत्र')]", 
            "//a[contains(text(), 'Issuance of Caste Certificate')]",
            "//a[contains(translate(text(), 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'caste')]"
        ],    
        "INCOME": [
            "//a[contains(text(), 'आय प्रमाण पत्र')]", 
            "//a[contains(text(), 'Issuance of Income Certificate')]",
            "//a[contains(translate(text(), 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'income')]"
        ]    
    }    
    ...
    # Block Level / अंचल स्तर पर Click
    find_and_interact(driver, [    
        "//a[contains(text(), 'अंचल स्तर पर')]",    
        "//a[contains(text(), 'Block Level')]",
        "//a[contains(text(), 'Revenue Officer Level')]"
    ])    
    
    # Safe Window / Tab Switch with explicit check
    try:
    ...
    # सुनिश्चित करें कि ServicePlus पेज का बॉडी लोड हो चुका है
    try:
    ...
    # Handle iFrame if present on form page
    ...
    gender_xpaths = [
        "//input[@id='gender1']", "//input[@value='M']", "//input[@name='gender' and @value='1']"
    ] if gender == "MALE" else [
        "//input[@id='gender2']", "//input[@value='F']", "//input[@name='gender' and @value='2']"
    ]    
