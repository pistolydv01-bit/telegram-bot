# ===================================================================
# Python Telegram Bot Source Code (Updated with Income Automation & Admin Panel)
# ===================================================================
...
# ===================================================================
# Centralized Error Screenshot Helper Function
# ===================================================================
...
# ===================================================================
# Step-Specific Screenshot Helper Function
# ===================================================================
...
def validate_rtps_json(service_type, user_data):
    ...
    if service_type == "RESIDENCE" and not user_data.get("residence_type"):      
        ...
        if not user_data.get("profession"):
            missing.append("profession")
        
        # कंपोनेंट या कुल आय में से कम से कम एक जानकारी आवश्यक है
        has_components = any([
            user_data.get("income_govt_service"),
            user_data.get("income_agriculture"),
            user_data.get("income_business"),
            user_data.get("income_other_sources")
        ])
        if not has_components and not user_data.get("annual_income"):
            missing.append("income components / annual_income")
...
# ===================================================================
# SPECIAL INCOME SECTION AUTOMATION FUNCTION
# ===================================================================

def fill_income_section(driver, user_data, timeout=20):
    """
    INCOME service के Others section को reliably fill करता है.

    Important:
    - चार component income fields भरता है।
    - Total Income (17173) को direct type नहीं करता।
    - input/change/blur events trigger करता है ताकि portal का
      JavaScript total calculate कर सके।
    """
    ...
    def get_income_value(key):
        ...
        # केवल numeric income स्वीकार करें
        value = re.sub(r"[^\d]", "", value)

        return value if value else "00"

    income_values = {
        "govt": get_income_value("income_govt_service"),
        "agriculture": get_income_value("income_agriculture"),
        "business": get_income_value("income_business"),
        "other": get_income_value("income_other_sources"),
    }

    wait = WebDriverWait(driver, timeout)

    for key, element_id in field_ids.items():
        value = income_values[key]
    ...
    # Portal को अपना calculation करने का मौका
    time.sleep(1)

    # Total field readonly है — केवल verify करें
    ...
    total_value = driver.execute_script(
        "return arguments[0].value;",
        total_element
    )

    logging.info(
        f"✅ Income components filled. "
        f"Portal Total Income = {mask_sensitive_data(str(total_value))}"
    )

    return total_value

# ===================================================================
# Helper: Mobile OTP XPaths
# ===================================================================
...
def fill_initial_rtps_form(driver, user_data, service_type, photo_path, chat_id=None, user_id=None):
    ...
    try:
        ...
        # 6. GRAM PANCHAYAT (ID = 56895)
        panchayat = str(user_data.get("panchayat", "")).strip()
        if panchayat:
            logging.info(f"📍 Waiting for Gram Panchayat = {panchayat} (56895)")
            gp = WebDriverWait(driver, 45).until(EC.presence_of_element_located((By.ID, "56895")))
    ...
    # ============================================================
    # SERVICE-SPECIFIC EXTRA FIELDS & INCOME HANDLING
    # ============================================================
    try:
        # Profession Field Common Dropdown
        if user_data.get("profession"):
            select_location_dropdown(driver, ["Profession", "पेशा"], user_data.get("profession"), timeout=8)
        
        # Application Purpose (Mostly used in INCOME certificates)
        if user_data.get("purpose_of_application"):
            try:
                find_and_interact(
                    driver, 
                    ["//input[contains(translate(@id,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'purpose') or contains(translate(@name,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'purpose')]"], 
                    "type", 
                    user_data.get("purpose_of_application"), 
                    chat_id=chat_id
                )
            except Exception as e:
                logging.warning(f"Purpose of Application input warning: {e}")

        # INCOME SERVICE INTEGRATION
        if service_type == "INCOME":
            logging.info("💰 INCOME service detected, executing fill_income_section...")
            fill_income_section(driver, user_data)
        else:
    ...
    if aadhaar_number:
        try:
            ...
            logging.info("✅ Identification number entered successfully.")
        except Exception as e:
            logging.warning(f"Identification number filling warning: {e}")
...
def admin_keyboard():
    ...
    markup.add(
        telebot.types.InlineKeyboardButton("👥 User List", callback_data="adm_list")
    )
    return markup
...
def admin_add_user(message):
    ...
    bot.reply_to(message, f"✅ User `{user_id}` successfully added.\n\nअब यह user bot चला सकता है।", parse_mode="Markdown")
...
def admin_remove_user(message):
    ...
    user_id = int(raw_id)
    if user_id in ALLOWED_USER_IDS:
        bot.reply_to(message, f"⚠️ `{user_id}` `.env` में मौजूद है।\n\nइसे Admin Panel से remove नहीं किया जाएगा।", parse_mode="Markdown")
        return
