# ===================================================================
# Python Telegram Bot Source Code (Updated with Gemini & Admin Panel)
# ===================================================================
...
# ===================================================================
# GEMINI DOCUMENT DATA EXTRACTION FUNCTION
# ===================================================================

def extract_rtps_data_with_gemini(file_path):
    """
    Document/PDF/Image से RTPS-related information निकालता है।
    Missing information को invent नहीं करता।
    """
...
Rules:
1. Extract only information actually visible/present in the document.
2. Never invent or guess a value.
3. If a field is not present, return an empty string.
4. Preserve names accurately.
5. Do not change one person's name into another person's name.
6. Preserve the distinction between applicant, father and mother.
7. Preserve digits accurately.
8. Return JSON only.
"""
    ...
    if not isinstance(data, dict):
        raise ValueError("Gemini ने valid JSON object नहीं दिया।")
...
# ===================================================================
# TEXT CLEANING & EXACT FILLING HELPER
# ===================================================================
...
def fill_text_exact(driver, element_id, value):
    ...
    if actual.strip() != value:
        raise ValueError(
            f"Field {element_id} mismatch: expected={value!r}, actual={actual!r}"
        )
    logging.info(f"✅ {element_id} = {actual!r}")
...
def fill_initial_rtps_form(driver, user_data, service_type, photo_path, chat_id=None, user_id=None):
    ...
    try:
        ...
        if panchayat:
            ...
            while time.time() < end_time:
                try:
                    ...
                    for option in options:
                        ...
                        if not text:
                            continue
                        if text.casefold() in ("please select", "select"):
                            continue
                        ...
                        if text_norm == target or target in text_norm or text_norm in target:
                            ...
                            selected_text = clean_text(selected.text)
                            selected_value = clean_text(selected.get_attribute("value"))

                            logging.info(f"🔎 Panchayat verification: text={selected_text!r}, value={selected_value!r}")
            ...
            if not selected_ok:
                try:
                    gp = driver.find_element(By.ID, "56895")
                    available = [clean_text(o.text) for o in Select(gp).options]
                except Exception:
                    available = []

                logging.error(f"❌ Panchayat select नहीं हुआ. Target={panchayat!r}; Available={available}")
    ...
    # ID Number Filling (EXACT ID 57339_txt)
    aadhaar_number = clean_text(user_data.get("aadhaar_number", ""))
    if aadhaar_number:
        try:
            find_and_interact(
                driver,
            ...
            logging.info("✅ ID number entered via 57339_txt")
        except Exception as e:
            logging.warning(f"ID number filling warning: {e}")
...
def process_service_type(message):
    ...
    bot.send_message(message.chat.id, f"सर्विस '{service}' चुनी गई। विवरण नीचे दिए गए JSON प्रारूप में भरें या सीधे फ़ोटो/Document भेजें:\n\n`{templates[service]}`", parse_mode="Markdown")      
...
def process_document_upload(message):
    ...
    # Attempt Gemini Extraction if Client Ready
    if gemini_client:
        bot.send_message(chat_id, "🤖 Gemini API द्वारा document से जानकारी निकाली जा रही है...")
        try:
            extracted = extract_rtps_data_with_gemini(doc_path)
            
            with bot.retrieve_data(user_id, chat_id) as data:
                existing_user_data = data.get('user_data', {})
                # Merge: Overwrite empty/missing fields with Gemini extracted data
                for k, v in extracted.items():
                    if v and not existing_user_data.get(k):
                        existing_user_data[k] = v
                data['user_data'] = existing_user_data

            preview_text = "📄 *Document से निकाली गई जानकारी:*\n\n"
            preview_text += f"👤 *Name:* {extracted.get('applicant_name', 'N/A')}\n"
            preview_text += f"👨 *Father:* {extracted.get('father_name', 'N/A')}\n"
            preview_text += f"👩 *Mother:* {extracted.get('mother_name', 'N/A')}\n"
            preview_text += f"📍 *District:* {extracted.get('district', 'N/A')}\n"
            preview_text += f"🏠 *Address:* {extracted.get('address', 'N/A')}\n"
            preview_text += f"📌 *Pin Code:* {extracted.get('pin_code', 'N/A')}\n\n"
            preview_text += "⚠️ कृपया ऊपर दी गई जानकारी सत्यापित करें।"

            markup = telebot.types.InlineKeyboardMarkup()
            markup.add(
                telebot.types.InlineKeyboardButton("✅ Confirm & Start Automation", callback_data="confirm_rtps_data"),
                telebot.types.InlineKeyboardButton("✏️ Edit JSON", callback_data="edit_rtps_data")
            )

            bot.send_message(chat_id, preview_text, parse_mode="Markdown", reply_markup=markup)
            bot.set_state(user_id, RTPSState.confirm_data, chat_id)
            return

        except Exception as e:
            logging.error(f"Gemini document extraction failed: {e}")
            bot.send_message(chat_id, f"⚠️ Document data extract करने में समस्या आई: {e}\nमौजूदा डिटेल्स के साथ प्रक्रिया जारी रखी जा रही है।")

    # Proceed directly if Gemini is unavailable or failed
    trigger_rtps_automation(chat_id, user_id)


@bot.callback_query_handler(func=lambda call: call.data in ["confirm_rtps_data", "edit_rtps_data"], state=RTPSState.confirm_data)
def process_data_confirmation_callback(call):
    chat_id = call.message.chat.id
    user_id = call.from_user.id

    if call.data == "confirm_rtps_data":
        bot.answer_callback_query(call.id, "✅ पुष्टि की गई!")
        bot.send_message(chat_id, "⚙️ RTPS ऑटोमेशन शुरू किया जा रहा है...")
        trigger_rtps_automation(chat_id, user_id)
    elif call.data == "edit_rtps_data":
        bot.answer_callback_query(call.id)
        with bot.retrieve_data(user_id, chat_id) as data:
            current = json.dumps(data.get('user_data', {}), indent=2, ensure_ascii=False)
        bot.send_message(chat_id, f"📝 कृपया सही JSON डेटा भेजें:\n\n`{current}`", parse_mode="Markdown")
