...
# -------------------------------------------------------------------    
# Direct RTPS Service URLs
# -------------------------------------------------------------------
SERVICE_URLS = {
    "RESIDENCE": "https://serviceonline.bihar.gov.in/renderApplicationForm.do?serviceId=4630012",
    "CASTE": "https://serviceonline.bihar.gov.in/renderApplicationForm.do?serviceId=4650013",
    "INCOME": "https://serviceonline.bihar.gov.in/renderApplicationForm.do?serviceId=4640012",
}
...
# -------------------------------------------------------------------    
# Updated find_and_interact with Detailed Diagnostics
# -------------------------------------------------------------------    
def find_and_interact(driver, xpaths, action_type="click", text_value=None, timeout=10):    
    last_err = None    
    for xp in xpaths:    
        try:    
            logging.info(f"🔎 TRYING XPATH: {xp}")
            ...
            logging.info(
                ...
                f"type={elem.get_attribute('type')}"
            )
    ...
    try:
        ...
        debug_path = "/tmp/rtps_debug.png"
        driver.save_screenshot(debug_path)
        logging.error(f"DEBUG SCREENSHOT: {debug_path}")
    except Exception as e:
        logging.error(f"Debug info error: {e}")

    raise NoSuchElementException(
        "Selector त्रुटि: सभी XPaths असफल रहे। "
...
# -------------------------------------------------------------------    
# RTPS Form Filling Logic (With Enhanced Page Debug Logs)
# -------------------------------------------------------------------    
def fill_initial_rtps_form(driver, user_data, service_type, photo_path):    
    url = SERVICE_URLS.get(service_type)

    if not url:
        raise ValueError(f"Invalid service type: {service_type}")

    logging.info(f"Opening RTPS service: {service_type}")
    ...
    logging.info("========== RTPS PAGE DEBUG ==========")
    logging.info(f"URL: {driver.current_url}")
    logging.info(f"TITLE: {driver.title}")
    logging.info(f"INPUT COUNT: {len(driver.find_elements(By.TAG_NAME, 'input'))}")
    logging.info(f"SELECT COUNT: {len(driver.find_elements(By.TAG_NAME, 'select'))}")
    logging.info(f"BUTTON COUNT: {len(driver.find_elements(By.TAG_NAME, 'button'))}")
    logging.info(f"IFRAME COUNT: {len(driver.find_elements(By.TAG_NAME, 'iframe'))}")
    logging.info("PAGE SOURCE LENGTH: " + str(len(driver.page_source)))
    logging.info("=====================================")
