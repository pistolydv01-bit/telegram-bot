...
def process_captcha_input(message):
    ...
    try:
        ...
        if alert_handled:
            ...
            # This is the exact problem shown in the user's RTPS screenshot:
            # the portal is rejecting the form data, NOT the CAPTCHA.
            if alert_kind == "FORM_VALIDATION":
                session["is_processing"] = False
                session["last_activity"] = time.time()
                send_step_screenshot(driver, chat_id, "RTPS_FORM_VALIDATION_ALERT")
                bot.send_message(
                    chat_id,
                    "⚠️ RTPS ने CAPTCHA को गलत नहीं बताया है।\n\n"
                    "📋 Form validation alert आया है:\n"
                    f"{alert_text}\n\n"
                    "❗ CAPTCHA attempt count नहीं बढ़ाया गया है।\n"
                    "🔧 ऊपर बताए Applicant Name / Mobile / Email की जाँच करें, "
                    "फिर सही details के साथ Proceed करें।\n"
                    "🧩 CAPTCHA अभी active हो तो वही code दोबारा दर्ज करने की जरूरत नहीं; "
                    "यदि portal नया CAPTCHA दिखाए तभी नया CAPTCHA दर्ज करें।"
                )
                bot.set_state(user_id, RTPSState.captcha_input, chat_id)
                return
        ...
        if step == "UNKNOWN":
            ...
            late_alert_handled, late_alert_text, late_alert_kind = (
                accept_rtps_verification_alert(driver, timeout=20)
            )
