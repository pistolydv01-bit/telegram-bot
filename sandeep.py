# ===================================================================
# Python Telegram Bot Source Code (Updated with Exact Income/Profession IDs)
# ===================================================================
...
def fill_initial_rtps_form(driver, user_data, service_type, photo_path, chat_id=None, user_id=None):
    ...
    # ============================================================
    # SERVICE-SPECIFIC EXACT ID MAPPINGS (Profession, Income, Purpose, Checkbox)
    # ============================================================
    try:
        # 1. Profession (ID: 17207)
        if user_data.get("profession"):
            logging.info("📍 Selecting Profession using exact ID 17207")
            select_location_dropdown(driver, ["17207", "Profession", "पेशा"], user_data.get("profession"), timeout=15)

        # 2. Purpose of Application (ID: 17172)
        if user_data.get("purpose"):
            logging.info("📍 Filling Purpose using exact ID 17172")
            find_and_interact(driver, ["//input[@id='17172']"], "type", user_data.get("purpose"), timeout=20, chat_id=chat_id)

        # 3. Income Fields (Govt, Agriculture, Business, Other Sources)
        if service_type == "INCOME" or user_data.get("income_govt_service") or user_data.get("income_agriculture") or user_data.get("income_business") or user_data.get("income_other_sources"):
            logging.info("📍 Filling Income fields with exact IDs...")
            
            # Govt Service Income (ID: 17168)
            find_and_interact(driver, ["//input[@id='17168']"], "type", user_data.get("income_govt_service", "00"), timeout=20, chat_id=chat_id)
            
            # Agriculture Income (ID: 17171)
            find_and_interact(driver, ["//input[@id='17171']"], "type", user_data.get("income_agriculture", "00"), timeout=20, chat_id=chat_id)
            
            # Business Income (ID: 17169)
            find_and_interact(driver, ["//input[@id='17169']"], "type", user_data.get("income_business", "00"), timeout=20, chat_id=chat_id)
            
            # Other Sources Income (ID: 17170)
            find_and_interact(driver, ["//input[@id='17170']"], "type", user_data.get("income_other_sources", "00"), timeout=20, chat_id=chat_id)
            
            # Total Annual Income (ID: 17173 - Readonly verification)
            try:
                total_income = driver.find_element(By.ID, "17173").get_attribute("value")
                logging.info(f"✅ Total Annual Income (Readonly ID 17173) = {total_income}")
            except Exception as e:
                logging.warning(f"Total income verification warning: {e}")

        # Fallback for generic annual_income if detailed income keys are missing
        elif user_data.get("annual_income"):
            find_and_interact(driver, ["//input[@id='17173']"], "type", user_data.get("annual_income"), timeout=20, chat_id=chat_id)

        # 4. Other Service Specific fields (Category, Caste, Residence Type)
        ...
        # 5. अन्यान्य अभिलेख Checkbox (ID: 94045_3)
        other_records = user_data.get("other_records", False)
        if other_records:
            try:
                ...
                logging.info("✅ 'अन्यान्य अभिलेख' checkbox selected via ID 94045_3")
            except Exception as e:
                logging.warning(f"Other records checkbox warning: {e}")
