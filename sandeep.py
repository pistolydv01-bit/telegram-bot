...
if ALLOWED_USER_IDS_ENV.strip():    
    try:    
        ALLOWED_USER_IDS = {int(uid.strip()) for uid in ALLOWED_USER_IDS_ENV.split(",") if uid.strip().isdigit()}    
    except Exception:    
        pass    
...
SESSION_TIMEOUT_SECONDS = 900       # 15 मिनट इनएक्टिविटी लिमिट    
MAX_SESSION_LIFETIME_SECONDS = 2700 # 45 मिनट हार्ड सेशन लिमिट    
...
def cleanup_user_files(chat_id):    
    ...
    if os.path.exists(user_dir):    
        try:    
            ...
            logging.error(f"Cleanup error occurred for chat_id {chat_id}")    
    ...
    try:    
        ...
        if actual_ext == '.pdf':    
            if not header.startswith(b'%PDF'):    
                return False, "अमान्य PDF फ़ाइल संरचना (Magic bytes mismatch)!"    
...
# -------------------------------------------------------------------    
# Driver Setup (Supports Custom Paths & Multiple Fallbacks)    
# -------------------------------------------------------------------    
def get_chrome_driver(chat_id):    
    ...
    if os.path.exists(bin_path):    
        ...
        logging.warning("⚠️ CHROME_BIN path missing, falling back to system default PATH.")    
...
# -------------------------------------------------------------------    
# Robust Element Interactor & Dependent Dropdown Handler    
# -------------------------------------------------------------------    
...
def select_dropdown_robustly(driver, element_by, locator_value, target_text, timeout=20, min_options=2):    
    if not target_text:    
        return    
    ...
    while time.time() < end_time:    
        try:    
            ...
            continue    
        time.sleep(0.5)    
    raise ValueError(f"ड्रॉपडाउन में विकल्प '{target_text}' लोड नहीं हो सका या नहीं मिला।")    
    
def find_and_interact(driver, xpaths, action_type="click", text_value=None, timeout=12):    
    wait = WebDriverWait(driver, timeout)    
    last_err = None    
    for xp in xpaths:    
        try:    
            ...
            last_err = e    
            continue    
    raise NoSuchElementException(f"Selector त्रुटि: सभी XPaths असफल रहे। Last err: {last_err}")    
...
# -------------------------------------------------------------------    
# RTPS Form Filling Logic    
# -------------------------------------------------------------------    
def fill_initial_rtps_form(driver, user_data, service_type, photo_path):    
    driver.get("https://serviceonline.bihar.gov.in/")    
    
    find_and_interact(driver, [    
        "//a[contains(normalize-space(.), 'सामान्य प्रशासन विभाग')]",    
        "//a[contains(text(), 'General Administration')]",    
        "//li[contains(@id, 'GAD')]//a"    
    ])    
    
    service_map = {    
        "RESIDENCE": ["//a[contains(text(), 'आवासीय प्रमाण पत्र')]", "//a[contains(text(), 'Issuance of Residence Certificate')]"],    
        "CASTE": ["//a[contains(text(), 'जाति प्रमाण पत्र')]", "//a[contains(text(), 'Issuance of Caste Certificate')]"],    
        "INCOME": ["//a[contains(text(), 'आय प्रमाण पत्र')]", "//a[contains(text(), 'Issuance of Income Certificate')]"]    
    }    
    find_and_interact(driver, service_map[service_type])    
    ...
    gender_xpaths = ["//input[@id='gender1']", "//input[@value='M']"] if gender == "MALE" else ["//input[@id='gender2']", "//input[@value='F']"]    
    ...
    find_and_interact(driver, ["//input[@id='applicant_name' or contains(@name, 'applicantName') or contains(@id, 'applicant_name_en')]"], "type", user_data["applicant_name"])    
    ...
    if user_data.get("mother_name"):    
        try:    
            ...
            pass    
    find_and_interact(driver, ["//input[@id='mobile_no' or contains(@name, 'mobile')]"], "type", user_data["mobile_no"])    
...
def route_to_next_step(bot, chat_id, user_id, driver, user_dir):    
    ...
    for _ in range(max_retries):    
        ...
        if step == "MOBILE_OTP":    
            ...
            bot.send_message(chat_id, "🔐 आधार OTP (UIDAI) दर्ज करें:")    
    ...
    if verify_submission_status(driver, timeout=3):    
        ...
        bot.send_message(chat_id, "⚠️ वर्तमान चरण की स्पष्ट पहचान नहीं हो सकी। कृपया ध्यान दें या दोबारा प्रयास करें।")    
...
def start_cmd(message):    
    ...
    if ALLOWED_USER_IDS and user_id not in ALLOWED_USER_IDS:    
        bot.send_message(message.chat.id, "❌ क्षमा करें, आप इस बॉट का उपयोग करने के लिए अधिकृत नहीं हैं।")    
        return    
    
    markup = telebot.types.ReplyKeyboardMarkup(one_time_keyboard=True, resize_keyboard=True)    
    markup.add("RESIDENCE", "CASTE", "INCOME")    
    bot.send_message(message.chat.id, "RTPS बिहार ऑटोमेशन बोट में आपका स्वागत है। कृपया अपनी सेवा चुनें:", reply_markup=markup)    
    bot.set_state(user_id, RTPSState.service_type, message.chat.id)    
...
def process_document_upload(message):    
    ...
    try:    
        ...
        try:    
            ...
            bot.send_message(chat_id, "❌ ब्राउज़र इनिशियलाइज़ेशन विफल हुआ।")    
            return    
        ...
        fill_initial_rtps_form(driver, user_data, service_type, photo_path)    
        ...
        bot.send_message(chat_id, "❌ ऑटोमेशन के दौरान तकनीकी त्रुटि हुई है।")    
        with session_lock:    
            session = active_user_sessions.get(chat_id)    
        if session:    
...
# -------------------------------------------------------------------    
# Input Handlers With Controlled Retry Counters    
# -------------------------------------------------------------------    
...
def process_mobile_otp_input(message):    
    ...
    try:    
        find_and_interact(driver, ["//input[@id='mobile_otp' or contains(@id, 'txtMobileOtp')]"], "type", otp_code)    
...
# -------------------------------------------------------------------    
# Final Submission Internal Implementation    
# -------------------------------------------------------------------    
def execute_final_submission_internal(chat_id, session, otp_code):    
    ...
    try:    
        ...
        if otp_code:    
            ...
            # Check if Aadhaar OTP retry is needed
        ...
        bot.send_message(chat_id, "❌ सबमिशन प्रक्रिया में त्रुटि हुई।")    
    finally:    
        if should_cleanup:    
            finish_and_cleanup_session(chat_id, session)    
        user_locks[chat_id].release()    
