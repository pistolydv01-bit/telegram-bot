...
def fill_initial_rtps_form(driver, user_data, service_type, photo_path, chat_id=None):    
    ...
    # 2. Robust Salutation Selection (Dropdown)
    if user_data.get("salutation"):    
        try:
            select_dropdown_robustly(
                driver, 
                By.XPATH, 
                "//select[contains(@id, 'salutation') or contains(@name, 'salutation')]", 
                user_data["salutation"], 
                min_options=1, 
                chat_id=chat_id
            )    
        except Exception as e:
    ...
    # 3. Applicant Name (Upgraded Robust XPaths)
    find_and_interact(driver, [
        "//input[@id='applicant_name']",
        "//input[contains(@id, 'applicant_name')]",
        "//input[contains(@name, 'applicantName')]",
        "//input[contains(@name, 'applicant_name')]"
    ], "type", user_data["applicant_name"], chat_id=chat_id)    
    
    find_and_interact(driver, [
        "//input[contains(@id, 'father_name') or contains(@name, 'fatherName') or contains(@name, 'father_name')]"
    ], "type", user_data["father_name"], chat_id=chat_id)    
    
    if user_data.get("mother_name"):    
        try:    
            find_and_interact(driver, [
                "//input[contains(@id, 'mother_name') or contains(@name, 'motherName') or contains(@name, 'mother_name')]"
            ], "type", user_data["mother_name"], chat_id=chat_id)    
        except Exception as e:    
            logging.warning(f"Mother name input warning: {e}")    
            
    find_and_interact(driver, [
        "//input[contains(@id, 'mobile_no') or contains(@name, 'mobile') or contains(@name, 'mobile_no')]"
    ...
    # 4. Upgraded Robust Local Body Selection with Warnings instead of silent passes
    ...
    # 5. Upgraded Panchayat Dropdown with higher timeout and min_options=2
