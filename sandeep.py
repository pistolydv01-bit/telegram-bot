...
def select_location_dropdown(driver, label_words, target_text, timeout=30):
    """
    RTPS dropdown को उसके visible label के आधार पर खोजता है। AJAX options आने तक wait करता है और selection verify करता है।
    """
        ...
        try:
            ...
            for select_elem in selects:
                try:
                    ...
                    # JS से इस select का सबसे नजदीकी field-container ढूँढें जिसमें सिर्फ यही select हो।
...
def fill_initial_rtps_form(driver, user_data, service_type, photo_path, chat_id=None, user_id=None):
    ...
    try:
        # STATE
        if not select_location_dropdown(driver, ["State", "राज्य"], "बिहार", timeout=30):
            raise Exception("State 'बिहार' select नहीं हुआ")
        time.sleep(1.5)

        # DISTRICT
        if user_data.get("district"):
            if not select_location_dropdown(driver, ["District", "जिला"], user_data["district"], timeout=30):
                raise Exception(f"District select नहीं हुआ: {user_data['district']}")
        time.sleep(1.5)

        # SUB-DIVISION
        if user_data.get("sub_division"):
            if not select_location_dropdown(driver, ["Sub-Division", "अनुमंडल"], user_data["sub_division"], timeout=30):
                raise Exception(f"Sub-Division select नहीं हुआ: {user_data['sub_division']}")
        time.sleep(1.5)

        # BLOCK
        if user_data.get("block"):
            if not select_location_dropdown(driver, ["Block", "प्रखंड"], user_data["block"], timeout=30):
        ...
        if panchayat:
            if not select_location_dropdown(driver, ["Panchayat", "ग्राम पंचायत"], panchayat, timeout=30):
        ...
        logging.info("✅ State/District/Sub-Division/Block completed.")
    except Exception as e:
        logging.error(f"❌ ADDRESS_FIELDS_ERROR: {e}")
        try:
