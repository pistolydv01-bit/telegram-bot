# ===================================================================
# Python Telegram Bot Source Code
# ===================================================================
...
# ===================================================================
# Helper: Comprehensive Mobile OTP XPaths (UPDATED FOR MODAL POPUP)
# ===================================================================

def get_mobile_otp_xpaths():
    return [
        # Modal popup specific inputs
        ...
        # Direct ID/Name/Placeholder variations
        "//input[@id='mobile_otp']",
...
def check_mobile_otp_result(driver, timeout=10):
    start = time.time()
    success_xpaths = [
        "//*[contains(translate(normalize-space(.), 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'otp verified')]",
        "//*[contains(translate(normalize-space(.), 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'otp verification successful')]",
        "//*[contains(normalize-space(.), 'OTP सत्यापित')]",
        "//*[contains(normalize-space(.), 'सफलतापूर्वक सत्यापित')]"
    ]
    error_xpaths = [
    ...
    while time.time() - start < timeout:  
        try:  
            ...
            logging.debug(f"OTP result check: {e}")  
        time.sleep(0.5)  
    return "UNKNOWN"

# ===================================================================
# Updated: Mobile OTP Popup Detector
# ===================================================================
...
def process_mobile_otp_input(message):
    ...
    try:      
        ...
        # Input box me OTP dalne ke baad thoda pause
        time.sleep(1)
        ...
        # -------------------------------------------------------------
        # 🔍 FIX: Validate Button Click karne ke liye Updated XPaths & Logic
        # -------------------------------------------------------------
        validate_btn_xpaths = [      
        ...
        time.sleep(1) # Popup settle hone ka wait
        
        # Click Validate button robustly using JS/Selenium click
        find_and_interact(      
            driver,      
            validate_btn_xpaths,      
            action_type="click",      
        ...
        # Response process hone के लिए 3-4 sekund wait karein taaki portal message update kare
        time.sleep(4)      
        
        # Screenshot taaki pata chale click ke baad kya hua
        debug_screenshot(driver, chat_id, "AFTER_VALIDATE_CLICK")

        otp_result = check_mobile_otp_result(driver, timeout=10)  
        ...
        if otp_result == "INVALID":  
            ...
            bot.send_message(  
                chat_id,   
                f"❌ OTP गलत है। पुनः OTP दर्ज करें (प्रयास {attempts}/{MAX_OTP_ATTEMPTS}):"  
            )  
            return  
        ...
        if otp_result != "SUCCESS":  
            ...
            bot.send_message(  
                chat_id,   
                "⚠️ OTP verification का स्पष्ट परिणाम नहीं मिला। अगला step शुरू नहीं किया गया है।"  
            )  
        ...
        # 📸 OTP wala page khatam hone ke baad new Page ka screenshot bot par bhejne ke liye:
