...
# -------------------------------------------------------------------    
# RTPS Form Filling Logic (Fixed Dropdown Accordion Navigation)
# -------------------------------------------------------------------    
def fill_initial_rtps_form(driver, user_data, service_type, photo_path, chat_id=None):    
    ...
    try:
        ...
        # 1. 'सामान्य प्रशासन विभाग' अकॉर्डियन मेन्यू को खोलें
        find_and_interact(driver, [
            "//*[contains(text(), 'सामान्य प्रशासन विभाग')]",
            "//a[contains(text(), 'General Administration')]",
            "//li[contains(@id, 'GAD')]//a",
            "//div[contains(@class, 'left_menu')]//li[1]//a"
        ], action_type="click", timeout=15)
        
        # सब-मेन्यू लोड होने के लिए 2 सेकंड रुकें
        time.sleep(2)
        
        # 2. सम्बंधित प्रमाण-पत्र सर्विस लिंक पर क्लिक करें
        service_links = {
            "RESIDENCE": [
                "//a[contains(text(), 'आवासीय प्रमाण-पत्र') or contains(text(), 'आवासीय प्रमाण पत्र')]",
                "//a[contains(@href, 'residence') or contains(@onclick, 'residence')]"
            ],
            "CASTE": [
                "//a[contains(text(), 'जाति प्रमाण-पत्र') or contains(text(), 'जाति प्रमाण पत्र')]",
                "//a[contains(@href, 'caste') or contains(@onclick, 'caste')]"
            ],
            "INCOME": [
                "//a[contains(text(), 'आय प्रमाण-पत्र') or contains(text(), 'आय प्रमाण पत्र')]",
                "//a[contains(@href, 'income') or contains(@onclick, 'income')]"
            ]
        }
        ...
        # 3. 'अंचल स्तर पर' (Block Level) लिंक चुनें
        logging.info("Clicking Block Level option...")
        find_and_interact(driver, [
            "//a[contains(text(), 'अंचल स्तर पर')]",
            "//a[contains(text(), 'Block Level')]",
            "//a[contains(@href, 'block') or contains(text(), 'अंचल')]"
        ], action_type="click", timeout=15)
        time.sleep(5)
        
        # नया विंडोज/टैब खुलने की स्थिति में स्विच करें
        if len(driver.window_handles) > 1:
            driver.switch_to.window(driver.window_handles[-1])
            
    except Exception as e:
        logging.error(f"Menu navigation failed: {e}")
        raise NoSuchElementException(f"Homepage menu se service link tak nahi pahunch sake: {e}")
