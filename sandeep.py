...
# RTPS: after the initial CAPTCHA + warning OK, keep waiting for Aadhaar OTP.
AADHAAR_OTP_WAIT_AFTER_CAPTCHA_SECONDS = int(os.getenv("AADHAAR_OTP_WAIT_AFTER_CAPTCHA_SECONDS", "90"))
MAX_OTP_ATTEMPTS = 4  
MAX_FILE_SIZE_MB = 5
...
def fill_final_email_mobile_and_wait_for_otp(driver, user_data, chat_id=None, user_id=None):
    ...
    if user_data.get("email"):
        ...
        try:
            ...
            if email_element is None:
                ...
                if chat_id:
                    send_step_screenshot(driver, chat_id, "EMAIL_FILLED")
                    bot.send_message(chat_id, "📧 Email भर दिया गया है। पूरा current screen screenshot Telegram पर भेज दिया गया है।")
    ...
    if chat_id:
        send_step_screenshot(driver, chat_id, "MOBILE_FILLED")
        bot.send_message(chat_id, "📱 Mobile number भर दिया गया है। पूरा current screen screenshot Telegram पर भेज दिया गया है।")
    time.sleep(1)
    ...
    if detect_mobile_otp_popup(driver, timeout=15):
        ...
        bot.send_message(chat_id, "📱 मोबाइल नंबर पर OTP भेजा गया है। कृपया OTP दर्ज करें:")
        send_step_screenshot(driver, chat_id, "MOBILE_OTP_POPUP_READY")
        if user_id:
            bot.set_state(user_id, RTPSState.mobile_otp_input, chat_id)
        return "MOBILE_OTP"
    return None
...
def send_masked_aadhaar_field_screenshot(driver, chat_id, user_dir):
    """Send a screenshot of the Aadhaar field with digits masked for privacy."""
    aadhaar_xpaths = [
        "//input[@id='57339_txt' and @data-datatype='AadhaarNumber']",
        "//input[@data-datatype='AadhaarNumber' and @name='57339_txt']",
        "//input[@data-verificationtype='2' and @data-type='AadhaarNumber']",
        "//input[contains(@name,'_txt') and @maxlength='16' and @verhoeff='16']"
    ]
    path = os.path.join(user_dir, f"aadhaar_field_refresh_{int(time.time()*1000)}.png")
    for xp in aadhaar_xpaths:
        try:
            ...
            if not elems:
                continue
            el = elems[0]
            original = el.get_attribute("value") or ""
            # Mask all but the last four digits before taking the screenshot.
            masked = ("*" * max(0, len(re.sub(r"\D", "", original)) - 4) +
                      re.sub(r"\D", "", original)[-4:]) if original else "(empty)"
            driver.execute_script(
                "arguments[0].setAttribute('data-original-value-for-screenshot', arguments[0].value);"
                "arguments[0].value = arguments[1];", el, masked
            )
            time.sleep(0.2)
            el.screenshot(path)
            # Restore the original field without firing any site event.
            driver.execute_script(
                "arguments[0].value = arguments[1];"
                "arguments[0].removeAttribute('data-original-value-for-screenshot');",
                el, original
            )
            with open(path, 'rb') as f:
                bot.send_photo(
                    chat_id, f,
                    caption="🪪 Aadhaar field (masked) — CAPTCHA Refresh के साथ current field का screenshot"
                )
            try:
                os.remove(path)
            except Exception:
            ...
            logging.debug("Masked Aadhaar field screenshot failed for %s: %r", xp, e)
    return False
...
def captcha_refresh_callback(call):
    ...
    try:
        ...
        if not refresh_captcha_image(driver, timeout=8):
            ...
            return
        # Refresh के साथ current Aadhaar field का masked screenshot भी भेजें.
        # Full screenshot पहले की तरह रखा गया है; Aadhaar field screenshot
        # privacy के लिए digits mask करके भेजा जाता है.
        send_step_screenshot(driver, chat_id, "CAPTCHA_AFTER_REFRESH_FULL_SCREEN")
        send_masked_aadhaar_field_screenshot(driver, chat_id, user_dir)
...
def route_to_next_step(bot, chat_id, user_id, driver, user_dir):
    ...
    for _ in range(max_retries):
        ...
        if step == "MOBILE_OTP":      
            ...
            send_step_screenshot(driver, chat_id, "EMAIL_OTP_POPUP_READY")
            bot.send_message(chat_id, "📧 ईमेल पर प्राप्त OTP दर्ज करें। Screenshot Telegram पर भेज दिया गया है:")      
...
def process_mobile_otp_input(message):
    ...
    try:      
        ...
        try:
            ...
            bot.send_message(chat_id, f"🔔 Mobile OTP alert: {mask_sensitive_data(alert_text)}")
            try:
                send_step_screenshot(driver, chat_id, "MOBILE_OTP_ALERT_BEFORE_OK")
            except Exception:
                pass
            alert.accept()
            send_step_screenshot(driver, chat_id, "MOBILE_OTP_AFTER_OK")
        except Exception:
            pass
...
def wait_for_post_captcha_verification_step(driver, timeout=None):
    """After the CAPTCHA Proceed + warning OK, wait without treating the old CAPTCHA as rejection.

    Priority: Aadhaar OTP -> Mobile OTP -> Email OTP. The old CAPTCHA is deliberately ignored
    during this wait because RTPS can keep the old CAPTCHA input in the DOM while opening the next modal.
    """
    if timeout is None:
        timeout = AADHAAR_OTP_WAIT_AFTER_CAPTCHA_SECONDS
    deadline = time.time() + timeout
    while time.time() < deadline:
        try:
            driver.switch_to.default_content()

            # Aadhaar is the expected next page/modal in this RTPS flow.
            for xp in get_aadhaar_otp_xpaths():
                for el in driver.find_elements(By.XPATH, xp):
                    if el.is_displayed() and el.is_enabled():
                        return "AADHAAR_OTP"

            # Keep these checks so the bot never gets stuck if RTPS routes to them first.
            for xp in get_mobile_otp_xpaths():
                for el in driver.find_elements(By.XPATH, xp):
                    if el.is_displayed() and el.is_enabled():
                        return "MOBILE_OTP"
            ...
            logging.debug("Waiting for post-CAPTCHA verification step: %r", e)
        time.sleep(0.5)
    return "TIMEOUT"
...
def process_captcha_input(message):
    """Process CAPTCHA, click Proceed, handle the RTPS warning OK, then wait for Aadhaar OTP."""
    chat_id = message.chat.id
    user_id = message.from_user.id
    if not user_is_allowed(user_id):
        return
    ...
    try:
        # 1) Fill the current visible CAPTCHA and send screenshot.
        ...
        # 2) Click Proceed only after the CAPTCHA has been entered.
        process_btn_xpaths = [
            "//input[@value='Proceed']",
        ...
        # 3) RTPS native warning/verification alert. Send the current screen,
        # capture the alert text, then OK it. Native browser chrome may not be
        # included in Selenium screenshots, so the exact alert text is also sent.
        send_step_screenshot(driver, chat_id, "WARNING_POPUP_SCREEN")
        alert_handled, alert_text = accept_rtps_verification_alert(driver, timeout=12)
        if alert_handled:
            bot.send_message(
                chat_id,
                "🔔 RTPS verification popup मिला।\n\n"
                f"{mask_sensitive_data(alert_text)}\n\n"
                "✅ Popup पर OK click कर दिया गया है।"
            )
            # Native browser alerts are not always included in Selenium screenshots;
            # still send the page screenshot immediately after OK.
            send_step_screenshot(driver, chat_id, "AFTER_WARNING_OK")
            session["last_activity"] = time.time()

        # 4) Some RTPS versions use an HTML modal with an OK button.
        html_ok = click_post_proceed_ok_if_present(driver, chat_id=chat_id, timeout=8)
        if html_ok:
            bot.send_message(chat_id, "✅ RTPS HTML warning/modal का OK भी click कर दिया गया है।")

        # 5) INITIAL CAPTCHA FLOW:
        #    Do NOT inspect the old CAPTCHA as a rejection here.
        #    Wait specifically for Aadhaar OTP, as requested.
        if not is_final_captcha:
            session["waiting_for"] = "AADHAAR_OTP"
            bot.send_message(
                chat_id,
                "⏳ Warning OK के बाद Aadhaar OTP page का इंतज़ार किया जा रहा है...\n"
                f"⏱️ अधिकतम {AADHAAR_OTP_WAIT_AFTER_CAPTCHA_SECONDS} सेकंड तक wait होगा।"
            )

            step = wait_for_post_captcha_verification_step(
                driver,
                timeout=AADHAAR_OTP_WAIT_AFTER_CAPTCHA_SECONDS
            )
            ...
            if step == "AADHAAR_OTP":
                ...
                session["waiting_for"] = "AADHAAR_OTP"
                session["last_activity"] = time.time()
                send_step_screenshot(driver, chat_id, "AADHAAR_OTP_PAGE_READY")
                bot.send_message(
                    chat_id,
                    "🔐 Aadhaar OTP page आ गया है।\n"
                    "📸 उसका पूरा screenshot Telegram पर भेज दिया गया है।\n"
                    "👉 अब Aadhaar पर आया OTP Telegram में भेजें।"
                )
                bot.set_state(user_id, RTPSState.aadhaar_otp_input, chat_id)
                return
            ...
            if step == "MOBILE_OTP":
                session["is_processing"] = False
                session["waiting_for"] = "MOBILE_OTP"
                session["last_activity"] = time.time()
                send_step_screenshot(driver, chat_id, "MOBILE_OTP_PAGE_READY")
                bot.send_message(chat_id, "📱 Mobile OTP page आ गया है। Screenshot Telegram पर भेज दिया गया है। OTP भेजें:")
                bot.set_state(user_id, RTPSState.mobile_otp_input, chat_id)
                return

            if step == "EMAIL_OTP":
                session["is_processing"] = False
                session["waiting_for"] = "EMAIL_OTP"
                session["last_activity"] = time.time()
                send_step_screenshot(driver, chat_id, "EMAIL_OTP_PAGE_READY")
                bot.send_message(chat_id, "📧 Email OTP page आ गया है। Screenshot Telegram पर भेज दिया गया है। OTP भेजें:")
                bot.set_state(user_id, RTPSState.email_otp_input, chat_id)
                return

            # Timeout is NOT a CAPTCHA rejection.
            session["is_processing"] = False
            session["last_activity"] = time.time()
            send_step_screenshot(driver, chat_id, "AADHAAR_OTP_WAIT_TIMEOUT")
            bot.send_message(
                chat_id,
                "⚠️ Warning OK हो गया, लेकिन Aadhaar OTP page अभी तक नहीं आया।\n"
                "❗ इसे CAPTCHA गलत नहीं माना गया है।\n"
                "📸 Current screen का screenshot ऊपर भेज दिया गया है।"
            )
            return

        # 6) FINAL CAPTCHA FLOW after Aadhaar verification.
        #    Wait for a real verification step; do not declare rejection merely
        #    because the old CAPTCHA input remains visible.
        session["last_activity"] = time.time()
        step = wait_for_post_captcha_verification_step(driver, timeout=20)

        if step == "AADHAAR_OTP":
            session["is_processing"] = False
            session["waiting_for"] = "AADHAAR_OTP"
            send_step_screenshot(driver, chat_id, "AADHAAR_OTP_PAGE_READY_AGAIN")
            bot.send_message(chat_id, "🔐 Aadhaar OTP page फिर से आ गया है। Screenshot भेज दिया गया है। OTP भेजें:")
            bot.set_state(user_id, RTPSState.aadhaar_otp_input, chat_id)
            return
        ...
        if step == "MOBILE_OTP":
            session["is_processing"] = False
            session["waiting_for"] = "MOBILE_OTP"
            send_step_screenshot(driver, chat_id, "MOBILE_OTP_PAGE_READY_AGAIN")
            bot.send_message(chat_id, "📱 Mobile OTP page आ गया है। Screenshot भेज दिया गया है। OTP भेजें:")
            bot.set_state(user_id, RTPSState.mobile_otp_input, chat_id)
            return
        ...
        if step == "EMAIL_OTP":
            session["is_processing"] = False
            session["waiting_for"] = "EMAIL_OTP"
            send_step_screenshot(driver, chat_id, "EMAIL_OTP_PAGE_READY_AGAIN")
            bot.send_message(chat_id, "📧 Email OTP page आ गया है। Screenshot भेज दिया गया है। OTP भेजें:")
            bot.set_state(user_id, RTPSState.email_otp_input, chat_id)
            return

        # No verification modal: final CAPTCHA/Proceed succeeded.
        session["awaiting_final_captcha"] = False
        session["waiting_for"] = "FINAL_SUBMISSION"
        session["is_processing"] = False
        session["last_activity"] = time.time()
        send_step_screenshot(driver, chat_id, "FINAL_PROCEED_SUCCESS_SCREEN")
        bot.send_message(
            chat_id,
            "✅ CAPTCHA सही था और Proceed सफल हुआ।\n"
...
def execute_final_submission_internal(chat_id, session, otp_code):
    ...
    try:
        ...
        if otp_code:
            ...
            send_step_screenshot(driver, chat_id, "AADHAAR_OTP_VALIDATE_CLICKED")
            ...
            if alert_handled:
                bot.send_message(chat_id, f"🔔 Aadhaar verification alert: {mask_sensitive_data(alert_text)}")
                logging.info(
                    "✅ Aadhaar verification alert accepted: %s",
                    mask_sensitive_data(alert_text)
                )
                send_step_screenshot(driver, chat_id, "AADHAAR_VERIFIED_AFTER_OK")
