...
def screenshot_before_after_rtps_alert(driver, chat_id, alert_kind="RTPS_ALERT"):
    """Send screenshots around the browser JavaScript alert.
    The first screenshot is taken while the alert is still present; the
    second is taken after alert.accept() returns.
    """
    if not chat_id:
        return
    try:
        send_step_screenshot(driver, chat_id, f"{alert_kind}_BEFORE_OK")
    except Exception as e:
        logging.warning("Pre-alert screenshot failed: %s", e)
...
def process_captcha_input(message):
    ...
    try:
        ...
        # Save the URL BEFORE Proceed so a real navigation can be detected.
        pre_proceed_url = ""
        try:
        ...
        # ------------------------------------------------------------
        # Screenshot A: the page immediately after CAPTCHA/Proceed, before
        # dismissing any RTPS JavaScript alert.
        try:
            send_step_screenshot(driver, chat_id, "CAPTCHA_PROCEED_ALERT_BEFORE_OK")
        except Exception as screenshot_error:
            logging.warning("Alert-before-OK screenshot warning: %s", screenshot_error)
        ...
        # Screenshot B: immediately after the browser alert has been accepted.
        if alert_handled:
            try:
                send_step_screenshot(driver, chat_id, "CAPTCHA_ALERT_AFTER_OK")
            except Exception as screenshot_error:
                logging.warning("Alert-after-OK screenshot warning: %s", screenshot_error)
        ...
        if alert_handled:
            ...
            # IMPORTANT: This RTPS message is an informational confirmation/
            # validation warning. The user specifically wants its OK button
            # clicked and the flow to continue. Do NOT stop the CAPTCHA flow
            # here and do NOT count it as a CAPTCHA failure.
            if alert_kind == "FORM_VALIDATION":
                logging.info(
                    "✅ RTPS form-validation/confirmation alert accepted; continuing flow: %s",
                    mask_sensitive_data(alert_text)
                )
                session["last_activity"] = time.time()
                bot.send_message(
                    chat_id,
                    "☑️ RTPS का ‘नाम / Mobile / Email’ वाला OK popup मिला।\n"
                    "✅ OK automatically click कर दिया गया है।\n"
                    "❗ इसे CAPTCHA गलत नहीं माना गया है।\n"
                    "⏳ अब OK के बाद अगला RTPS step check किया जा रहा है..."
                )
                time.sleep(2)
        ...
        # Screenshot C: actual page reached after the alert/verification
        # transition. This is intentionally after state detection so it is
        # not just the old CAPTCHA DOM.
        try:
            send_step_screenshot(
                driver, chat_id,
                "NEXT_PAGE_AFTER_CAPTCHA_ALERT"
            )
        except Exception as screenshot_error:
            logging.warning("Next-page screenshot warning: %s", screenshot_error)
        ...
        if step == "UNKNOWN":
            ...
            if step == "UNKNOWN":
                ...
                bot.send_message(
                    chat_id,
                    "⚠️ Proceed पर click हो चुका है।\n"
                    "☑️ अगर RTPS ने confirmation/validation popup दिखाया था तो उसका OK automatically click किया जा चुका है।\n"
                    "❗ CAPTCHA को गलत नहीं माना गया है।\n"
                    "⏳ RTPS का अगला step अभी detect नहीं हुआ।\n"
                    "🔎 अगर popup के बाद भी यही form रहे, तो RTPS ने किसी form detail को validate नहीं किया है; यह CAPTCHA error नहीं है।"
                )
                bot.set_state(user_id, RTPSState.captcha_input, chat_id)
                return
...
def execute_final_submission_internal(chat_id, session, otp_code=""):
    ...
    try:
        ...
        # Screenshot D: draft/acknowledgement page before opening Annexure.
        send_step_screenshot(driver, chat_id, "DRAFT_PAGE_BEFORE_ATTACH_ANNEXURE")
        ...
        # Screenshot E: Annexure page after Attach Annexure click.
        send_step_screenshot(driver, chat_id, "ANNEXURE_PAGE_AFTER_ATTACH")
        ...
        # Screenshot F: both Annexure rows are selected and files are attached,
        # immediately before Save Annexure.
        send_step_screenshot(driver, chat_id, "ANNEXURE_READY_BEFORE_SAVE")
        ...
        # Screenshot G: the page after Save Annexure has completed.
        send_step_screenshot(driver, chat_id, "ANNEXURE_AFTER_SAVE")
        ...
        bot.send_message(
            ...
            "⏹️ आपकी बताई flow के अनुसार Save Annexure के बाद यहीं रुक रहा है..."
        )

        bot.send_message(
            chat_id,
            "✅ दोनों Annexure दस्तावेज़ Save Annexure से save हो गए हैं।\n"
            "📸 Save के बाद का पूरा screenshot ऊपर भेज दिया गया है।\n"
            "⏹️ आपकी बताई हुई flow यहीं रोकी गई है।"
        )
