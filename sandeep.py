...
def process_captcha_input(message):
    ...
    try:
        # ------------------------------------------------------------
        # 1) Fill the exact RTPS CAPTCHA answer field.
        # ------------------------------------------------------------
        ...
        # CAPTCHA code भरने के बाद भी पूरा screenshot Telegram पर भेजें,
        # ताकि साफ दिखे कि user ने क्या भरा और form की बाकी स्थिति क्या है।
        send_step_screenshot(driver, chat_id, "CAPTCHA_CODE_FILLED_FULL_SCREEN")

        # ------------------------------------------------------------
        # 2) Click the exact RTPS Proceed button.
        # ------------------------------------------------------------
        process_btn_xpaths = [
        ...
        # USER REQUEST:
        # Proceed click होते ही पूरा page screenshot Telegram पर भेजें.
        time.sleep(1)
        send_step_screenshot(
            driver,
            chat_id,
            "AFTER_CAPTCHA_PROCEED_CLICK"
        )
        # Also send the actual CAPTCHA image currently shown on the page.
        current_captcha_path = os.path.join(
            user_dir, f"captcha_after_proceed_{int(time.time()*1000)}.png"
        )
        if capture_captcha_image(driver, get_captcha_image_xpaths(), current_captcha_path):
            try:
                with open(current_captcha_path, "rb") as c_img:
                    bot.send_photo(
                        chat_id, c_img,
                        caption="🧩 Proceed के तुरंत बाद page पर दिख रहा actual CAPTCHA."
                    )
            finally:
                try:
        ...
        # ------------------------------------------------------------
        # 4) If CAPTCHA is still present, DO NOT automatically refresh.
        #    The CAPTCHA currently visible after Proceed is the important
        #    screenshot the user asked to receive on Telegram.
        # ------------------------------------------------------------
        if step == "CAPTCHA":
            ...
            # Server rejection के तुरंत बाद पूरा page screenshot भेजें।
            # इसे refresh से पहले भेजा जाता है, इसलिए यही actual rejected state है।
            send_step_screenshot(driver, chat_id, "CAPTCHA_REJECTED_FULL_SCREEN")

            # Send the ACTUAL CURRENT CAPTCHA after the server rejected
            # the submitted answer. Do this BEFORE any refresh.
            current_captcha_path = os.path.join(
                user_dir,
            ...
            if capture_captcha_image(
                    ...
                    with open(current_captcha_path, "rb") as c_img:
                        bot.send_photo(
                            ...
                            caption=(
                                ...
                                "🧩 यह वही CAPTCHA है जो Proceed के बाद अभी page पर दिख रहा है।\n"
                ...
                bot.send_message(
                    chat_id,
                    "❌ CAPTCHA verify नहीं हुआ, लेकिन current CAPTCHA image capture नहीं हो सकी।"
                )

            # IMPORTANT: Do NOT call refresh_captcha_image() here.
            # Refresh is now manual via Telegram button, so the screenshot
            # above corresponds to the CAPTCHA actually present on the page.
        ...
        # ------------------------------------------------------------
        # 5) CAPTCHA accepted -> continue normally.
        # ------------------------------------------------------------
        session["is_processing"] = False
        session["last_activity"] = time.time()
        bot.send_message(
