...
def find_and_interact(driver, xpaths, action_type="click", text_value=None, timeout=20, chat_id=None):    
    ...
    while time.time() - start_time < timeout:
        ...
        for xp in xpaths:    
            try:    
                ...
                if visible_elems:
                    return _perform_action(driver, visible_elems[0], action_type, text_value)
            except Exception as e:    
                last_err = e    
                continue    
    ...
    raise NoSuchElementException(
        f"Selector त्रुटि: {timeout} सेकंड में कोई भी XPath नहीं मिला। Last error: {last_err}"
    )
...
def fill_initial_rtps_form(driver, user_data, service_type, photo_path, chat_id=None):    
    ...
    find_and_interact(driver, gender_xpaths, chat_id=chat_id)    
    
    if user_data.get("salutation"):    
        try:
            select_dropdown_robustly(driver, By.XPATH, "//select[@id='salutation' or contains(@name, 'salutation')]", user_data["salutation"], min_options=1, chat_id=chat_id)    
        except Exception:
            pass

    find_and_interact(driver, ["//input[@id='applicant_name' or contains(@name, 'applicantName') or contains(@id, 'applicant_name_en') or contains(@name, 'name')]"], "type", user_data["applicant_name"], chat_id=chat_id)    
    find_and_interact(driver, ["//input[@id='father_name' or contains(@name, 'fatherName') or contains(@id, 'father_name_en')]"], "type", user_data["father_name"], chat_id=chat_id)    
    
    if user_data.get("mother_name"):    
        try:    
            find_and_interact(driver, ["//input[@id='mother_name' or contains(@name, 'motherName') or contains(@id, 'mother_name_en')]"], "type", user_data["mother_name"], chat_id=chat_id)    
        except Exception:    
            pass    
            
    find_and_interact(driver, ["//input[@id='mobile_no' or contains(@name, 'mobile') or contains(@id, 'mobile')]"], "type", user_data["mobile_no"], chat_id=chat_id)    
    
    if user_data.get("email"):    
        try:    
            find_and_interact(driver, ["//input[@id='email' or contains(@name, 'email')]"], "type", user_data["email"], chat_id=chat_id)    
        except Exception:    
            pass    
    ...
    if local_body in local_body_xpaths:    
        try:    
            find_and_interact(driver, [local_body_xpaths[local_body]], chat_id=chat_id)    
            time.sleep(1)    
        except Exception:    
            pass    
    
    if local_body == "GRAM_PANCHAYAT" and user_data.get("panchayat"):    
        select_dropdown_robustly(driver, By.XPATH, "//select[@id='panchayat' or contains(@name, 'panchayat')]", user_data.get("panchayat"), chat_id=chat_id)    
...
def process_document_upload(message):    
    ...
    try:    
        ...
        try:
            ...
            send_error_screenshot(chat_id, driver, "❌ Automation Critical Failure", e)
            session = active_user_sessions.get(chat_id)
            if session:
    ...
    # Lock release होने के बाद route_to_next_step कॉल करना सुरक्षित है
