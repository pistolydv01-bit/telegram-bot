...
def process_captcha_input(message):
    ...
    try:
        ...
        # IMPORTANT: submit_btn is reused by RTPS on several pages (CAPTCHA,
        # Attach Annexure, Save Annexure). Therefore the generic ID selector
        # must NOT be the first selector. Target the actual visible Proceed
        # button first.
        process_btn_xpaths = [
        ...
        # Log the exact target so a bad duplicate submit_btn can no longer be
        # silently selected.
        try:
            driver.switch_to.default_content()
            proceed_candidates = driver.find_elements(By.XPATH, process_btn_xpaths[0])
            proceed_candidates = [e for e in proceed_candidates if e.is_displayed() and e.is_enabled()]
            if not proceed_candidates:
                proceed_candidates = driver.find_elements(By.XPATH, process_btn_xpaths[1])
                proceed_candidates = [e for e in proceed_candidates if e.is_displayed() and e.is_enabled()]
            if proceed_candidates:
                _pb = proceed_candidates[0]
                logging.info(
                    "🎯 RTPS CAPTCHA Proceed target: tag=%s id=%s text=%r onclick=%r disabled=%r",
                    _pb.tag_name, _pb.get_attribute('id'), _pb.text,
                    _pb.get_attribute('onclick'), _pb.get_attribute('disabled')
                )
        except Exception as e:
        ...
        if step == "UNKNOWN":
            ...
            if step == "UNKNOWN":
                ...
                if step == "UNKNOWN":
                    session["is_processing"] = False
                    session["last_activity"] = time.time()
                    # This screenshot is a diagnostic screenshot only. It must
                    # never be described as NEXT_PAGE unless Annexure/OTP was
                    # actually detected.
                    send_step_screenshot(driver, chat_id, "CAPTCHA_PROCEED_NO_STATE_CHANGE")
                    bot.send_message(
                        chat_id,
                        "⚠️ Proceed और alert-OK पूरा हो गया है, लेकिन RTPS ने अभी next page नहीं दिखाया।\n"
                        "📸 Current page का diagnostic screenshot भेजा गया है।\n"
                        "❗ CAPTCHA को गलत नहीं माना गया है।\n"
                        "⏳ कृपया RTPS transition/response का इंतज़ार करें या नया CAPTCHA आने पर उसे दर्ज करें।"
                    )
                    bot.set_state(user_id, RTPSState.captcha_input, chat_id)
                    return
