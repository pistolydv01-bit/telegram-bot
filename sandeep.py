...
def detect_current_verification_step(driver, timeout=8):
    ...
    while time.time() - start_time < timeout:
        try:
            ...
            # Mobile OTP (Using get_mobile_otp_xpaths)
...
def fill_initial_rtps_form(driver, user_data, service_type, photo_path, chat_id=None, user_id=None):    
    ...
    # 2. Click Email field first (to trigger RTPS 'Validate OTP' popup if applicable)
...
def continue_rtps_form_after_mobile_otp(driver, user_data, service_type, photo_path, chat_id=None, user_id=None):
    try:
        # Type email now that OTP is verified / popup handled
        if user_data.get("email"):
            try:
                find_and_interact(driver, [
...
def process_mobile_otp_input(message):    
    ...
    try:    
        # --------------------------------------------------    
        # 1. Make sure we are on main document    
        # --------------------------------------------------    
        try:    
            driver.switch_to.default_content()    
        except Exception:    
        ...
        # --------------------------------------------------    
        # 4. Wait for RTPS response    
        # --------------------------------------------------    
        time.sleep(2)    
        step = detect_current_verification_step(    
            driver,    
            timeout=5    
        )    
        logging.info( f"Verification step after Mobile OTP: {step}" )    
        ...
        if step == "MOBILE_OTP":    
            ...
            if attempts >= MAX_OTP_ATTEMPTS:    
                bot.send_message(    
                    chat_id, f"❌ अधिकतम मोबाइल OTP प्रयास ({MAX_OTP_ATTEMPTS}) समाप्त हुए।"    
                )    
                finish_and_cleanup_session(    
                    chat_id, session    
                )    
                return    
            bot.send_message(    
                chat_id, f"❌ अमान्य OTP!\nपुनः OTP दर्ज करें (प्रयास {attempts}/{MAX_OTP_ATTEMPTS}):"    
            )    
            return    
        
        # --------------------------------------------------    
        # 6. Mobile OTP successful    
        # --------------------------------------------------    
        session["is_processing"] = False    
        session["last_activity"] = time.time()    
        bot.send_message(    
        ...
        # --------------------------------------------------    
        # 9. Go to next verification step    
        # --------------------------------------------------    
        route_to_next_step(    
