...
# -------------------------------------------------------------------    
# Direct RTPS Service URLs & Fallbacks
# -------------------------------------------------------------------
SERVICE_URLS = {
    "RESIDENCE": "[https://serviceonline.bihar.gov.in/renderApplicationForm.do?serviceId=4630012](https://serviceonline.bihar.gov.in/renderApplicationForm.do?serviceId=4630012)",
    "CASTE": "[https://serviceonline.bihar.gov.in/renderApplicationForm.do?serviceId=4650013](https://serviceonline.bihar.gov.in/renderApplicationForm.do?serviceId=4650013)",
    "INCOME": "[https://serviceonline.bihar.gov.in/renderApplicationForm.do?serviceId=4640012](https://serviceonline.bihar.gov.in/renderApplicationForm.do?serviceId=4640012)",
}
...
# -------------------------------------------------------------------    
# Enhanced Robust Interaction Engine
# -------------------------------------------------------------------    
def check_and_switch_iframe(driver, xpaths):
    """
    सुरक्षित रूप से सभी iFrames की जांच करता है और एलिमेंट मिलते ही उस Frame में रुक जाता है।
    """
    try:
...
def find_and_interact(driver, xpaths, action_type="click", text_value=None, timeout=20):    
    ...
    # 1. पहले Main Page (DOM) में ढूंढने की कोशिश करें
    ...
    # 2. अगर Main Page पर नहीं मिला, तो iFrames में सर्च करें
...
def fill_initial_rtps_form(driver, user_data, service_type, photo_path):    
    ...
    driver.get(url)    
    time.sleep(6) # RTPS पोर्टल लोड होने के लिए पर्याप्त समय

    # Check for direct landing or main site redirect
    if "serviceonline.bihar.gov.in" in driver.current_url and len(driver.find_elements(By.TAG_NAME, "input")) < 3:
        logging.info("Re-attempting via Main RTPS Portal Navigation...")
        ...
        try:
            ...
            service_links = {
                "RESIDENCE": "//a[contains(text(), 'आवासीय प्रमाण पत्र')]",
                "CASTE": "//a[contains(text(), 'जाति प्रमाण पत्र')]",
                "INCOME": "//a[contains(text(), 'आय प्रमाण पत्र')]"
            }
            find_and_interact(driver, [service_links[service_type]])
            time.sleep(2)
            find_and_interact(driver, ["//a[contains(text(), 'अंचल स्तर पर')]", "//a[contains(text(), 'Block Level')]"])
            time.sleep(4)
            
            if len(driver.window_handles) > 1:
                driver.switch_to.window(driver.window_handles[-1])
        except Exception as e:
            logging.warning(f"Navigation error, falling back to direct URL: {e}")
            driver.get(url)
            time.sleep(6)
