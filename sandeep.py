...
def safe_step(bot, chat_id, step_name, func, is_mandatory=False):
    """
    हर एक form step को safely execute करने के लिए wrapper.
    अगर exception आता है, तो bot क्रैश नहीं होगा बल्कि Telegram पर 'Next Step' बटन भेजेगा।
    """
    try:
        ...
        # Telegram Inline Keyboard with Next Step Button
        markup = telebot.types.InlineKeyboardMarkup()
        markup.add(
            telebot.types.InlineKeyboardButton("➡️ Next Step", callback_data="rtps_next_step")
        )
        
        # Mandatory vs Optional warning message
        mandatory_warning = (
...
def continue_rtps_form_after_mobile_otp(
    ...
    try:
        ...
        if chat_id:
            ...
            bot.send_message(chat_id, "🔄 फॉर्म डिटेल्स भरना शुरू हो रहा है...")

        # 1. Email (Optional/Semi-mandatory)
        safe_step(
            bot, chat_id, "EMAIL", 
            lambda: (
                lambda f=(
                ...
                ): (f.clear(), f.send_keys(user_data.get("email", "")))
            )(), 
            is_mandatory=False
        )
        ...
        safe_step(
            bot, chat_id, "STATE", 
            lambda: select_dropdown_robustly(
                driver, By.XPATH, "//select[contains(translate(@id,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'state') or contains(translate(@name,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'state')]", "बिहार", timeout=8, chat_id=chat_id
            ), 
            is_mandatory=True
        )

        # 3. District (Mandatory)
        safe_step(
            bot, chat_id, "DISTRICT", 
            lambda: select_dropdown_robustly(
                driver, By.XPATH, "//select[contains(translate(@id,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'district') or contains(translate(@name,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'district')]", user_data.get("district"), timeout=8, chat_id=chat_id
            ), 
            is_mandatory=True
        )

        # 4. Sub-Division (Mandatory)
        safe_step(
            bot, chat_id, "SUB-DIVISION", 
            lambda: select_dropdown_robustly(
                driver, By.XPATH, "//select[contains(translate(@id,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'sub') or contains(translate(@name,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'sub')]", user_data.get("sub_division"), timeout=8, chat_id=chat_id
            ), 
            is_mandatory=True
        )
        ...
        safe_step(
            bot, chat_id, "BLOCK", 
            lambda: select_dropdown_robustly(
                driver, By.XPATH, "//select[contains(translate(@id,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'block') or contains(translate(@name,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'block') or contains(translate(@id,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'circle')]", user_data.get("block"), timeout=8, chat_id=chat_id
            ), 
            is_mandatory=True
        )

        # 6. Local Body Type (Optional/Conditional)
        safe_step(
            bot, chat_id, "LOCAL BODY", 
            lambda: select_dropdown_robustly(
                driver, By.XPATH, "//select[contains(translate(@id,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'localbody') or contains(translate(@name,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'localbody') or contains(translate(@id,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'bodytype')]", user_data.get("local_body_type"), timeout=8, chat_id=chat_id
            ), 
            is_mandatory=False
        )

        # 7. Gram Panchayat (Optional in urban areas, Mandatory in rural)
        safe_step(
            bot, chat_id, "GRAM PANCHAYAT", 
            lambda: select_dropdown_robustly(
                driver, By.XPATH, "//select[contains(translate(@id,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'panchayat') or contains(translate(@name,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'panchayat')]", user_data.get("panchayat"), timeout=8, chat_id=chat_id
            ), 
            is_mandatory=False
        )

        # 8. Ward (Optional/Conditional)
        safe_step(
            bot, chat_id, "WARD", 
            lambda: (
                lambda f=(
                ...
                ): (f.clear(), f.send_keys(user_data.get("ward_no", "")))
            )(), 
            is_mandatory=False
        )

        if chat_id:
            send_step_screenshot(driver, chat_id, "AFTER_MOBILE_OTP_FORM_FIELDS")
...
def process_document_upload(message):
    ...
    try:      
        ...
        if not valid:      
            bot.send_message(chat_id, "❌ दस्तावेज़ त्रुटि: {err_msg}")      
            return      
