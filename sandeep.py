...
def process_user_details(message):
    ...
    try:      
        ...
        with bot.retrieve_data(user_id, message.chat.id) as data:      
            ...
            saved_other_doc = data.get("other_doc_path")

        if gemini_pending and saved_photo and saved_doc:
            # Aadhaar document is the first document.  The second mandatory
            # annexure ("अन्यान्य अभिलेख") is collected separately before
            # starting browser automation.
            if not saved_other_doc:
                with bot.retrieve_data(user_id, message.chat.id) as data:
                    data["gemini_review_pending"] = False
                bot.send_message(
                    message.chat.id,
                    "✅ JSON सत्यापित हुआ। अब दूसरा दस्तावेज़ भेजें:\n\n"
                    "📄 Document 2: अन्यान्य अभिलेख (JPG/PNG/PDF)\n"
                    "यह Annexure में दूसरे mandatory स्थान पर upload होगा।"
                )
            ...
            with bot.retrieve_data(user_id, message.chat.id) as data:
                data["gemini_review_pending"] = False
            bot.send_message(message.chat.id, "✅ दोनों दस्तावेज़ मिल गए। अब RTPS automation शुरू किया जा रहा है...")
...
def start_saved_rtps_automation(chat_id, user_id):
    ...
    try:
        with bot.retrieve_data(user_id, chat_id) as data:
            ...
            doc_path = data.get('aadhaar_doc_path') or data.get('doc_path', '')
...
def process_document_upload(message):
    ...
    try:      
        ...
        # Document 1 = Aadhaar. Keep a dedicated path for the Annexure step.
        doc_path = os.path.join(user_dir, f"aadhaar_doc{ext}")
        downloaded = bot.download_file(file_info.file_path)
        ...
        if os.getenv("GEMINI_AUTO_EXTRACT", "true").strip().lower() in {"1", "true", "yes", "on"}:
            try:
                with bot.retrieve_data(user_id, chat_id) as data:
                    ...
                    data["aadhaar_doc_path"] = doc_path
        ...
        # If Gemini is disabled/fails, the first uploaded file is still Aadhaar.
        # Collect the second mandatory annexure before opening the browser.
        with bot.retrieve_data(user_id, chat_id) as data:
            data["aadhaar_doc_path"] = doc_path
            data["doc_path"] = doc_path
            data["gemini_review_pending"] = False
        ...
        bot.send_message(
            chat_id,
            "📄 Aadhaar document प्राप्त हुआ। अब दूसरा दस्तावेज़ भेजें:\n\n"
            "Document 2: अन्यान्य अभिलेख (JPG/PNG/PDF)"
        )
    ...
    try:
        ...
        while time.time() < deadline:
            try:
                ...
                if detected in ("MOBILE_OTP", "EMAIL_OTP"):
                    step = detected
                    break
                if detected == "CAPTCHA":
                    step = "CAPTCHA"
