# ===================================================================
# Python Telegram Bot Source Code (Updated with OTP Stability Fixes)
# ===================================================================
...
MAX_OTP_ATTEMPTS = 4  # Portal JavaScript allows globalCountValidate < 4
MAX_FILE_SIZE_MB = 5
...
def wait_after_mobile_otp(driver, timeout=20):
    try:
        wait = WebDriverWait(driver, timeout)
        # OTP modal hidden होना चाहिए
        wait.until(
            lambda d: not any(
                e.is_displayed() for e in d.find_elements(By.ID, "digilockerPopUp")
            )
        )
        # Backdrop हटना चाहिए
        wait.until(
            lambda d: len(
                d.find_elements(By.CSS_SELECTOR, ".modal-backdrop.show, .modal-backdrop")
            ) == 0
        )
        logging.info("✅ OTP modal closed and backdrop removed")
        return True
    except Exception as e:
        logging.error("OTP post-processing wait failed: %s", e)
        return False
...
def check_mobile_otp_result(driver, timeout=20):
    ...
    while time.time() - start < timeout:
        try:
            ...
            # 1. Check for explicit error messages on page or inside modal
            error_xpaths = [
            ...
            # 2. Check reliable DOM state via wait_after_mobile_otp
...
def continue_rtps_form_after_mobile_otp(
    ...
    try:
        ...
        # 1. OTP modal/overlay पूरी तरह हटने का इंतज़ार
        ...
        # 2. EMAIL (यदि फॉर्म में उपलब्ध हो - वैकल्पिक हो सकता है)
        try:
            email_field = wait.until(EC.presence_of_element_located(
                (By.XPATH, "//input[contains(translate(@id,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'email') or contains(translate(@name,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'email')]")
            ))
            email_field.clear()
            ...
            logging.info("Email field not found or optional, skipping.")

        # 3. STATE (अनिवार्य फील्ड - एरर आने पर रुक जाएगा)
        select_dropdown_robustly(
            driver,
            By.XPATH,
            "//select[contains(translate(@id,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'state') or contains(translate(@name,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'state')]",
            "बिहार",
            chat_id=chat_id
        )
        if chat_id:
            send_step_screenshot(driver, chat_id, "STATE_SELECTED")

        # 4. DISTRICT (अनिवार्य फील्ड)
        select_dropdown_robustly(
            driver,
            By.XPATH,
            "//select[contains(translate(@id,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'district') or contains(translate(@name,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'district')]",
            user_data.get("district"),
            chat_id=chat_id
        )
        if chat_id:
            send_step_screenshot(driver, chat_id, "DISTRICT_SELECTED")

        # 5. SUB-DIVISION (अनिवार्य फील्ड)
        select_dropdown_robustly(
            driver,
            By.XPATH,
            "//select[contains(translate(@id,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'sub') or contains(translate(@name,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'sub')]",
            user_data.get("sub_division"),
            chat_id=chat_id
        )
        if chat_id:
            send_step_screenshot(driver, chat_id, "SUB_DIVISION_SELECTED")

        # 6. BLOCK / CIRCLE (अनिवार्य फील्ड)
        select_dropdown_robustly(
            driver,
            By.XPATH,
            "//select[contains(translate(@id,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'block') or contains(translate(@name,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'block') or contains(translate(@id,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'circle')]",
            user_data.get("block"),
            chat_id=chat_id
        )
        if chat_id:
            send_step_screenshot(driver, chat_id, "BLOCK_SELECTED")

        # 7. LOCAL BODY / RURAL-URBAN TYPE (अनिवार्य फील्ड)
        select_dropdown_robustly(
            driver,
            By.XPATH,
            "//select[contains(translate(@id,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'localbody') or contains(translate(@name,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'localbody') or contains(translate(@id,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'bodytype')]",
            user_data.get("local_body_type"),
            chat_id=chat_id
        )
        if chat_id:
            send_step_screenshot(driver, chat_id, "LOCAL_BODY_SELECTED")

        # 8. PANCHAYAT / WARD (ग्रामीण या शहरी के आधार पर अनिवार्य)
        try:
            select_dropdown_robustly(
                driver,
                By.XPATH,
                "//select[contains(translate(@id,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'panchayat') or contains(translate(@name,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'panchayat')]",
                user_data.get("panchayat"),
                chat_id=chat_id
            )
            if chat_id:
                send_step_screenshot(driver, chat_id, "PANCHAYAT_SELECTED")
        except Exception as e:
            logging.warning("Panchayat dropdown not found or skipped for urban area.")

        # 9. WARD FILLED (यदि लागू हो)
        try:
            ward_field = wait.until(EC.presence_of_element_located(
                (By.XPATH, "//input[contains(translate(@id,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'ward') or contains(translate(@name,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'ward')]")
            ))
            ward_field.clear()
            ward_field.send_keys(user_data.get("ward_no", ""))
            if chat_id:
                send_step_screenshot(driver, chat_id, "WARD_FILLED")
        except Exception:
            logging.info("Ward field not found or not required.")

        # 10. BEFORE SUBMIT (फॉर्म सबमिट करने से ठीक पहले का चेकपॉइंट)
        if chat_id:
            send_step_screenshot(driver, chat_id, "BEFORE_SUBMIT")

        # 11. FINAL SUBMIT ACTION (या अगले पेज पर जाने का बटन)
        submit_btn = wait.until(EC.element_to_be_clickable(
            (By.XPATH, "//button[contains(translate(@id,'ABCDEFGHIJKLMNOPQRSTUVWXYZ','abcdefghijklmnopqrstuvwxyz'),'submit') or contains(text(), 'Submit') or contains(text(), 'Proceed') or contains(@id, 'next')]")
        ))
        submit_btn.click()

        time.sleep(3)
        if chat_id:
            send_step_screenshot(driver, chat_id, "AFTER_SUBMIT")
...
# ===================================================================
# UPDATED: process_mobile_otp_input Function with Integrated Checkpoints
# ===================================================================
...
def process_mobile_otp_input(message):
    ...
    try:      
        ...
        # Handle Portal's alert and check if "Verified" is present
        ...
        try:
            send_step_screenshot(driver, chat_id, "AFTER_MOBILE_OTP_CLOSED")
        except Exception:
            pass
        ...
        session["last_activity"] = time.time()  
        bot.send_message(  
            chat_id, "✅ Mobile OTP सत्यापित हो गया। फॉर्म आगे भरा जा रहा है..."  
        )  
        ...
        session["is_processing"] = False      
        session["last_activity"] = time.time()      
        logging.exception("Mobile OTP processing error")      
        bot.send_message(      
            chat_id, f"❌ OTP प्रक्रिया में त्रुटि हुई:\n{e}"      
        )      
        send_error_screenshot(      
            chat_id, driver, "❌ Mobile OTP Error", e      
        )
