# ===================================================================
# Python Telegram Bot Source Code (Updated with Gemini Flash Flow)
# ===================================================================
...
# ============================================================
# GEMINI MODEL & CLIENT CONFIGURATION
# ============================================================
...
if GEMINI_API_KEY:
    ...
    gemini_client = None
    logging.warning("⚠️ GEMINI_API_KEY .env फ़ाइल में उपलब्ध नहीं है!")
...
def extract_rtps_data_with_gemini(file_path, service_type):
    ...
    if service_type not in JSON_TEMPLATES:
        raise ValueError(
            f"Unsupported service: {service_type}"
        )

    template = JSON_TEMPLATES[service_type]

    prompt = f"""
इस official document से RTPS form के लिए उपलब्ध जानकारी निकालो।
...
8. Selected service के fields भी उसी structure में रखो।
...
def fill_initial_rtps_form(driver, user_data, service_type, photo_path, chat_id=None, user_id=None):
    ...
    try:
        ...
        if village:
            ...
            logging.info("✅ Village/Mohalla filled successfully")
        ...
        if ward:
            ...
            logging.info("✅ Ward No. filled successfully")
        ...
        if post_office:
            ...
            logging.info("✅ Post Office filled successfully")
        ...
        if police_station:
            ...
            logging.info("✅ Police Station selected successfully")
        ...
        if pin_code:
            ...
            logging.info("✅ Pin Code filled successfully")
    ...
    if aadhaar_number:
        try:
            ...
            logging.info("✅ Identity number entered via 57339_txt")
        except Exception as e:
            logging.warning(f"Identity number filling warning: {e}")
...
def process_service_type(message):
    ...
    bot.send_message(message.chat.id, f"सर्विस '{service}' चुनी गई। अब सपोर्टिंग दस्तावेज़ (PDF/JPG/PNG) भेजें ताकि Gemini AI द्वारा जानकारी निकाली जा सके:")      
...
def process_document_upload(message):
    ...
    try:      
        ...
        bot.send_message(chat_id, "🔍 Gemini Flash द्वारा दस्तावेज़ से जानकारी निकाली जा रही है...")      
        ...
        logging.error(f"Document extraction error in chat {chat_id}: {e}")
        bot.send_message(chat_id, f"❌ Gemini Data Extraction में त्रुटि:\n{e}")
