...
# ==========================================
# 🛑 ADMIN DETAILS & UPI SETTINGS
# ==========================================
ADMIN_ID = 6874667015               # Yahan apni Telegram Numeric ID dalein
ADMIN_USERNAME = "ooooooo929"       # Aapka Telegram Username
PAYMENT_UPI_ID = "sandeepkumar960148.rzp@rxairtel" # Yahan apna sahi UPI ID dalein
# ==========================================
...
def get_user_credits(user_id, referrer_id=None):
    ...
    uid = str(user_id)
    if uid not in data:
        data[uid] = {"credits": 1, "used": 0, "referred_by": referrer_id}
        save_data(data)
...
def add_credits_to_user(user_id, count):
    ...
    uid = str(user_id)
    if uid not in data:
        data[uid] = {"credits": 0, "used": 0, "referred_by": None}
...
def send_welcome(message):
    ...
    msg = (
        ...
        f"Aapke pass <b>{credits}</b> search credit(s) baaki hain.\n\n"
...
# --- Callbacks for Subscriptions & Direct QR Code ---
...
def callback_query(call: CallbackQuery):
    if call.data == "buy_sub":
        ...
        # Automatic Dynamic UPI QR Code Generation
