...
async function startWhatsApp() {
    ...
    waSock.ev.on('connection.update', (update) => {
        ...
        if (connection === 'open') {
            ...
            if (shouldReconnect) // Health-check HTTP server (replaces the separate sandip.py health server)
...
async function checkWhatsAppNumber(cleaned) {
    return Promise.race([
        waSock.onWhatsApp(cleaned),
        new Promise((_, reject) =>
            setTimeout(() => reject(new Error('WhatsApp check timeout')), CHECK_TIMEOUT_MS)
        )
    ]);
}
...
async function processScanning(chatId, numbers) {
    if (!waSock) {
        return bot.sendMessage(
            chatId,
            "⚠️ **WhatsApp Engine ready nahi hai. Thodi der baad prayas karein.**",
            { parse_mode: 'Markdown' }
        );
    }
    ...
    if (uniqueNumbers.length === 0) {
        return bot.sendMessage(chatId, "⚠️ Koi valid number nahi mila.");
    }
    ...
    await bot.sendMessage(
        chatId,
        `⚙️ **Checking ${uniqueNumbers.length} numbers...**\n\n` +
        `⏱️ Har number ka maximum wait: ${CHECK_TIMEOUT_MS / 1000}s`,
        { parse_mode: 'Markdown' }
    );

    for (const cleaned of uniqueNumbers) {
        try {
            const resultList = await checkWhatsAppNumber(cleaned);
        ...
        if (processed % PROGRESS_EVERY === 0 || processed === uniqueNumbers.length) {
            const percent = Math.floor((processed / uniqueNumbers.length) * 100);
            await bot.sendMessage(
                chatId,
                `🔄 **Progress:** ${processed}/${uniqueNumbers.length} (${percent}%)\n` +
                `✅ Registered: ${registered.length}\n` +
                `❌ Not registered/error: ${notRegistered.length}`,
                { parse_mode: 'Markdown' }
            );
        }

        if (processed < uniqueNumbers.length) {
            await sleep(DELAY_MS);
        }
    }

    const reportText =
        `📊 **WhatsApp Check Report**\n\n` +
